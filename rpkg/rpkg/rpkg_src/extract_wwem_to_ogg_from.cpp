                        ...
                        for (uint64_t j = 0; j < rpkgs.at(i).hashes_indexes_based_on_resource_types.at(r).size(); j++)
                        {
                            ...
                            if (!extract_single_hash || (extract_single_hash && filter == rpkgs.at(i).hash.at(hash_index).hash_string))
                            {
                                ...
                                if (it != hash_list_hash_map.end())
                                {
                                    ...
                                    size_t pos2 = hash_list_string.find(".wav].pc_wem");
                                    ...
                                    if (pos1 != std::string::npos && pos2 != std::string::npos && (pos2 - pos1) > 12)
                                    {
                                        ...
                                        size_t pos3 = wwem_ioi_path.find_last_of("\\");

                                        wwem_base_name = wwem_ioi_path.substr(pos3 + 1);

                                        wwem_ioi_directory = wwem_ioi_path.substr(0, pos3);
                                ...
                                if (wwem_ioi_path_found)
                                {
                                    ...
                                    if (found || filter == "")
                                    {
                                        ...
                                        std::ofstream wwem_meta_data_file = std::ofstream(wwem_meta_data_file_name, std::ifstream::binary);
