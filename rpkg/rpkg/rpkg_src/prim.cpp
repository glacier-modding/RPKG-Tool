...
prim::prim(uint64_t rpkgs_index, uint64_t hash_index)
{
    ...
    asset3ds_data.name = prim_file_name;
    ...
    for (uint32_t d = 0; d < 0x30; d++)
    {
        prim_meta_data.push_back(prim_data.data()[prim_position + d]);
    }
    ...
    for (uint32_t o = 0; o < prim_object_offsets.size(); o++)
    {
        ...
        if (header_type_value == (uint16_t)prim_header::header_type::MESH)
        {
            if (object_sub_type_value == (uint16_t)prim_object::SUBTYPE::STANDARD)
            {
                ...
                for (uint32_t d = 0; d < 0x50; d++)
                {
                    prim_meta_data.push_back(prim_data.data()[prim_position + d]);
                }

                prim_sub_mesh temp_prim_sub_mesh(prim_data, prim_position, o, temp_prim_mesh, prim_meta_data, primary_prim_object_header.flag_high_resolution);
            ...
            else if (object_sub_type_value == (uint16_t)prim_object::SUBTYPE::WEIGHTED || object_sub_type_value == (uint16_t)prim_object::SUBTYPE::LINKED)
            {
                ...
                prim_weighted_sub_mesh temp_prim_weighted_sub_mesh(prim_data, prim_position, o, temp_prim_weighted_mesh, prim_meta_data, primary_prim_object_header.flag_high_resolution);
...
prim::prim_sub_mesh::prim_sub_mesh(std::vector<char>& prim_data, uint32_t& prim_position, uint32_t prim_object_number, prim_mesh& temp_prim_mesh, std::vector<char>& meta_data, bool flag_high_resolution)
{
    char input[1024];
    char char4[4];
    uint8_t bytes1 = 0;
    ...
    if (uv_channel_count == 1)
    {
        ...
        if (flag_high_resolution)
        {
            for (uint32_t v = 0; v < vertex_count; v++)
            {
                ...
                vertexes.push_back(temp_vector4);
            }
        }
        else
        {
        ...
        if (1)//if (prim_object_sub_type.at(o) != 0x2)
        {
            for (uint32_t v = 0; v < vertex_count; v++)
            {
                ...
                vertexes_normals.push_back(temp_vector4);
                ...
                vertexes_tangents.push_back(temp_vector4);
                ...
                vertexes_bitangents.push_back(temp_vector4);
                ...
                vertexes_uvs.push_back(temp_vector2);
            ...
            if (temp_prim_mesh.prim_object_instance.sub_type == (uint32_t)prim_object::SUBTYPE::WEIGHTED || ((prim_object_instance.property_flags & (uint32_t)prim_object::PROPERTY_FLAGS::COLOR1) == 0x0))
            {
                for (uint32_t v = 0; v < vertex_count; v++)
                {
                    ...
                    vertexes_colors.push_back(temp_rgba);
                }
            }

            for (uint32_t d = 0; d < 0x4; d++)
            {
            ...
            if (cloth_offset)
            {
                ...
                if ((temp_prim_mesh.cloth_flags & (uint32_t)prim_mesh::CLOTH_FLAGS::SMALL) == (uint32_t)prim_mesh::CLOTH_FLAGS::SMALL)
                {
                    ...
                    std::memcpy(&char4, &cloth_data_size, sizeof(bytes4));
                    for (uint32_t d = 0; d < sizeof(bytes4); d++)
                    {
                        meta_data.push_back(char4[d]);
                    }

                    for (uint32_t d = 0; d < cloth_data_size; d++)
                    {
                        meta_data.push_back(prim_data.data()[prim_position + d]);
                    }
                }
                else
                {
            ...
            if (collision_offset)
            {
                ...
                if (temp_prim_mesh.prim_object_instance.sub_type == (uint32_t)prim_object::SUBTYPE::STANDARD || temp_prim_mesh.prim_object_instance.sub_type == (uint32_t)prim_object::SUBTYPE::WEIGHTED)
                {
                    ...
                    std::memcpy(&char4, &collision_data_size, sizeof(bytes4));
                    for (uint32_t d = 0; d < sizeof(bytes4); d++)
                    {
                        meta_data.push_back(char4[d]);
                    }

                    for (uint32_t d = 0; d < collision_data_size; d++)
                    {
                        meta_data.push_back(prim_data.data()[prim_position + d]);
                    }
                }
...
prim::prim_weighted_sub_mesh::prim_weighted_sub_mesh(std::vector<char>& prim_data, uint32_t& prim_position, uint32_t prim_object_number, prim_weighted_mesh& temp_weighted_prim_mesh, std::vector<char>& meta_data, bool flag_high_resolution)
{
    char input[1024];
    char char4[4];
    uint8_t bytes1 = 0;
    ...
    if (uv_channel_count == 1)
    {
        ...
        {
            uint32_t weighted_data_size = vertex_count * 0xC;

            std::memcpy(&char4, &weighted_data_size, sizeof(bytes4));
            for (uint32_t d = 0; d < sizeof(bytes4); d++)
            {
                meta_data.push_back(char4[d]);
            }

            for (uint32_t d = 0; d < weighted_data_size; d++)
            {
                meta_data.push_back(prim_data.data()[prim_position + d]);
            }
            ...
            for (uint32_t v = 0; v < vertex_count; v++)
            {
                ...
                vertexes_weighted_weights.push_back(temp_vector6);
                ...
                vertexes_weighted_bone_ids.push_back(temp_uint8_t_6);
            }
        }
        else
        {
        ...
        if (1)//if (prim_object_sub_type.at(o) != 0x2)
        {
            ...
            if (temp_weighted_prim_mesh.prim_mesh_instance.prim_object_instance.sub_type != (uint32_t)prim_object::SUBTYPE::STANDARD)
            {
                if (temp_weighted_prim_mesh.bones_nodes_count)
                {
                    ...
                    uint32_t bones_nodes_data_size_bytes = bones_nodes_data_size * 4;

                    std::memcpy(&char4, &bones_nodes_data_size_bytes, sizeof(bytes4));
                    for (uint32_t d = 0; d < sizeof(bytes4); d++)
                    {
                        meta_data.push_back(char4[d]);
                    }

                    for (uint32_t d = 0; d < bones_nodes_data_size_bytes; d++)
                    {
                        meta_data.push_back(prim_data.data()[prim_position + d]);
                    }
                ...
                if (temp_weighted_prim_mesh.bone_info_offset)
                {
                    ...
                    std::memcpy(&char4, &bone_info_data_size, sizeof(bytes4));
                    for (uint32_t d = 0; d < sizeof(bytes4); d++)
                    {
                        meta_data.push_back(char4[d]);
                    }

                    for (uint32_t d = 0; d < bone_info_data_size; d++)
                    {
                        meta_data.push_back(prim_data.data()[prim_position + d]);
                    }
                ...
                if (temp_weighted_prim_mesh.bone_indices_offset)
                {
                    ...
                    uint32_t bone_indices_data_size = bytes2;
                    bone_indices_data_size -= 2;

                    uint32_t bone_indices_data_size_bytes = bone_indices_data_size * 2;

                    std::memcpy(&char4, &bone_indices_data_size_bytes, sizeof(bytes4));
                    for (uint32_t d = 0; d < sizeof(bytes4); d++)
                    {
                        meta_data.push_back(char4[d]);
                    }

                    for (uint32_t d = 0; d < bone_indices_data_size_bytes; d++)
                    {
                        meta_data.push_back(prim_data.data()[prim_position + d]);
                    }
...
void prim::extract_meta(std::string output_path)
{
    ...
    output_file.write(prim_meta_data.data(), prim_meta_data.size());
