...
void rpkg_function::extract_direct_hash_depends(std::string& input_path, std::string& filter, std::string& output_path)
{
        ...
        for (uint64_t x = 0; x < hashes_to_extract.size(); x++)
        {
            for (uint64_t i = 0; i < rpkgs.size(); i++)
            {
                ...
                if (it != rpkgs.at(i).hash_map.end())
                {
                    ...
                    rpkg_vars.output_path = file::output_path_append("DIRECTDEPENDS\\" + filter_hash_string + "\\", output_path);
