                ...
                if (rpkgs.at(i).hash_resource_types.at(r) == "LOCR")
                {
                    for (uint64_t j = 0; j < rpkgs.at(i).hashes_indexes_based_on_resource_types.at(r).size(); j++)
                    {
                        ...
                        if (found || filter == "")
                        {
                            ...
                            if (((locr_current_count * (uint64_t)100000) / locr_count) % static_cast<uint64_t>(100) == 0 &&
                                locr_current_count > 0)
                            {
                                stringstream_length = console::update_console(
                                    message, locr_count, locr_current_count, start_time, stringstream_length);
                            }

                            locr_current_count++;

                            std::string current_path = file::output_path_append(
                                "LOCR\\" + rpkgs.at(i).rpkg_file_name, output_path);
                            ...
                            for (uint64_t k = 0; k < (number_of_languages * (uint64_t)0x4 + (isLOCRv2
                                     ? (uint64_t)0x1
                                     : (uint64_t)0x0)); k++)
                            {
                                ...
                                if (language_offsets.at(k) != 0xFFFFFFFF)
                                {
                                    ...
                                    for (uint64_t l = 0; l < language_string_count.back(); l++)
                                    {
                                        ...
                                        if (symKey)
                                        {
                                            ...
                                            temp_json_object2["String"] = std::string(
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                LOG_AND_EXIT(
                                                    "Error: LOCR file " + hash_file_name + " in " + rpkgs.at(i).
                                                    rpkg_file_name + " is malformed.");
                                            }
                                            ...
                                            for (uint32_t m = 0; m < temp_language_string_length.back() / 8; m++)
                                            {
                                                ...
                                                std::memcpy(data, &temp_string[(uint64_t)m * (uint64_t)8],
                                                            sizeof(uint32_t));
                                                std::memcpy(
                                                    data + 1, &temp_string[(uint64_t)m * (uint64_t)8 + (uint64_t)4],
                                                    sizeof(uint32_t));
                                            ...
                                            std::string temp_string_with_zero_pad_removed = std::string(
                                                temp_string.begin(), temp_string.end()).substr(0, last_zero_position);
