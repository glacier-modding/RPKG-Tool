...
void generic_function::command_json(std::string& json_file_path)
{
    ...
    for (const auto& it : command_json.items())
    {
        ...
        if (it.value().contains("Command"))
        {
            ...
            else if (util::to_upper_case(it.value()["Command"]) == "SEARCH_RPKG")
            {
                std::string command = "-search_rpkg";
                ...
                {
                    if (util::to_upper_case(it.value()["SearchType"]) == "TEXT")
                    {
                        if (it.value().contains("SearchString"))
                        {
                            search = std::string(it.value()["SearchString"]);
                        }
                        else
                        {
                            LOG_AND_EXIT("Error: Command search_rpkg requires a \"SearchString\" value in the input JSON.");
                        }
                    }
                    else if (util::to_upper_case(it.value()["SearchType"]) == "HEX")
                    {
                        if (it.value().contains("SearchString"))
                        {
                            search = std::string(it.value()["SearchString"]);
                    ...
                    else if (util::to_upper_case(it.value()["SearchType"]) == "REGEX")
                    {
                        if (it.value().contains("SearchString"))
                        {
                            search = std::string(it.value()["SearchString"]);
                    ...
                    {
                        LOG_AND_EXIT("Error: \"SearchType\" value in the input JSON is not one of the only three valid options: text, hex, or regex");
                    }
                }
                else
                {
                    LOG_AND_EXIT("Error: Command search_rpkg requires a \"SearchType\" value in the input JSON.");
                }
                ...
                if (it.value().contains("InputFilePath"))
                {
                    ...
                    if (util::to_upper_case(it.value()["SearchType"]) == "TEXT")
                    {
                        search_type = "text";
                        LOG("  SearchType: Text");
                        LOG("  SearchString: " << it.value()["SearchString"]);
                    }
                    else if (util::to_upper_case(it.value()["SearchType"]) == "HEX")
                    {
                        search_type = "hex";
                ...
                {
                    LOG_AND_EXIT("Error: Command search_rpkg requires a \"InputFilePath\" value in the input JSON.");
                }
            }
