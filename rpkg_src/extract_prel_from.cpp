                ...
                if (rpkgs.at(i).hash_resource_types.at(r) == "PREL")
                {
                    for (uint64_t j = 0; j < rpkgs.at(i).hashes_indexes_based_on_resource_types.at(r).size(); j++)
                    {
                        ...
                        for (uint64_t k = 0; k < master_hash_depends_count; k++)
                        {
                            ...
                            std::memcpy(&temp_master_hash_depend_hash_value, &prel_data->data()[position],
                                        sizeof(uint64_t));
                            position += 0x8;

                            master_hash_depends.push_back(temp_master_hash_depend_hash_value);

                            std::cout << "Master hash depend (" << k << "): " << util::uint64_t_to_hex_string(
                                master_hash_depends.back()) << std::endl;

                            std::string ioi_string = "";

                            std::map<uint64_t, uint64_t>::iterator it2 = hash_list_hash_map.find(
                                master_hash_depends.back());
