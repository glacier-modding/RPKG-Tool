...
public:
    ...
    void generate_map_node_strings(uint32_t root_map_node, uint32_t parent_map_node, std::string map_node_string, uint32_t level);
	...
	static void import_map(std::string& input_path, std::string& filter, std::string& output_path);
	static void export_map(std::string& input_path, std::string& filter, std::string& output_path, bool textured);
    void add_to_m_eidParent_local_temp_hash_entityIndex_map(uint32_t map_node, uint64_t temp_hash, uint32_t entityIndex);
    void add_to_m_eidParent_external_externalSceneIndex_hash_entityID_map(uint32_t map_node, uint64_t externalSceneIndex_hash, uint32_t entityID);
    ...
    std::string godot_v3_tscn_file;
    std::string godot_v4_tscn_file;
