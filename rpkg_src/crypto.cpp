...
void crypto::xtea_encrypt_localization(uint32_t v[2])
{
    ...
    {
        v0 += (((v1 << 4) ^ (v1 >> 5)) + v1) ^ (sum + localization_keys[sum & 3]);
        sum += localization_delta;
        v1 += (((v0 << 4) ^ (v0 >> 5)) + v0) ^ (sum + localization_keys[(sum >> 11) & 3]);
    }
    v[0] = v0;
    v[1] = v1;
}
...
{
    uint32_t v0 = v[0], v1 = v[1], sum = decrypt_sum;
    for (uint64_t i = 0; i < num_rounds; i++)
    {
        v1 -= (v0 << 4 ^ v0 >> 5) + v0 ^ sum + localization_keys[sum >> 11 & 3];
        sum -= localization_delta;
...
void crypto::xtea_encrypt_packagedefinition_thumbs(uint32_t v[2])
{
    ...
    for (uint64_t i = 0; i < num_rounds; i++)
    {
        v0 += (v1 << 4 ^ v1 >> 5) + v1 ^ sum + thumbs_keys[sum & 3];
        sum -= thumbs_delta;
        v1 += (v0 << 4 ^ v0 >> 5) + v0 ^ sum + thumbs_keys[sum >> 11 & 3];
    }
    v[0] = v0;
    v[1] = v1;
}
...
{
    const int bits[] = {value & 1, (value & 2) >> 1, (value & 4) >> 2, (value & 8) >> 3, (value & 16) >> 4, (value & 32) >> 5, (value & 64) >> 6, (value & 128) >> 7};
    const int output = bits[0] << 0 | (bits[1] << 4) | (bits[2] << 1) | (bits[3] << 5) | (bits[4] << 2) | (bits[5] << 6) | (bits[6] << 3) | (bits[7] << 7);
    return output ^ 226;
}
