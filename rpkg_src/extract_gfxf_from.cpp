...
void rpkg_function::extract_gfxf_from(std::string& input_path, std::string& filter, std::string& output_path)
{
            ...
            for (uint64_t r = 0; r < rpkgs.at(i).hash_resource_types.size(); r++)
            {
                if (rpkgs.at(i).hash_resource_types.at(r) == "GFXF")
                {
                    for (uint64_t j = 0; j < rpkgs.at(i).hashes_indexes_based_on_resource_types.at(r).size(); j++)
                    {
                        ...
                        if (((gfxf_count_current * static_cast<uint64_t>(100000)) / (uint64_t)gfxf_count) % static_cast<
                            uint64_t>(10) == 0 && gfxf_count_current > 0)
                        {
                            stringstream_length = console::update_console(
                                message, gfxf_hash_size_total, gfxf_hash_size_current, start_time, stringstream_length);
                        }
                        ...
                        if (!extract_single_hash || (extract_single_hash && filter == rpkgs.at(i).hash.at(hash_index).
                            hash_string))
                        {
                            std::string current_path = file::output_path_append(
                                "GFXF\\" + rpkgs.at(i).rpkg_file_name, output_path);
                            ...
                            else
                            {
                                ...
                                {
                                    LOG_AND_EXIT(
                                        "Error: DDS file " + gfxf_output_dir + "\\" + hash_file_name + ".GFX" +
                                ...
                                if (gfxf_contains_dds)
                                {
                                    ...
                                    for (uint64_t d = 0; d < dds_names_count; d++)
                                    {
                                        ...
                                        std::memcpy(&input, &gfxf_data->data()[dds_name_offset],
                                                    (static_cast<uint64_t>(dds_name_length) + static_cast<uint64_t>(
                                                        1)));
                                    ...
                                    for (uint64_t d = 0; d < dds_names_count; d++)
                                    {
                                        ...
                                        std::ofstream output_file = std::ofstream(
                                            gfxf_output_dir + "\\" + dds_names.at(d), std::ofstream::binary);

                                        if (!output_file.good())
                                        {
                                            LOG_AND_EXIT(
                                                "Error: DDS file " + gfxf_output_dir + "\\" + dds_names.at(d) +
                                                " could not be created.");
                                        }
                                ...
                                std::ofstream gfx_meta_output_file = std::ofstream(
                                    gfxf_output_dir + "\\" + "meta", std::ofstream::binary);

                                if (!gfx_meta_output_file.good())
                                {
                                    LOG_AND_EXIT(
                                        "Error: DDS file " + gfxf_output_dir + "\\" + "meta" +
                                        " could not be created.");
                                }
                                ...
                                std::string final_path = gfxf_output_dir + "\\" + rpkgs.at(i).hash.at(hash_index).
                                    hash_file_name;
    ...
    ss << message << "100% Done in " << (0.000000001 * std::chrono::duration_cast<
        std::chrono::nanoseconds>(end_time - start_time).count()) << "s";
