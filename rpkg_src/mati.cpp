        ...
        if (!has_diffuse_texture)
        {
            if (mati_properties.at(p).string_value == "mapTexture2D_01")
            {
                for (uint32_t r = 0; r < mati_properties.size(); r++)
                {
                    ...
                    {
                        if (log_output)
                            std::cout << "Diffuse texture found at index: " << mati_properties.at(r).int_value << std::endl;

                        if (mati_properties.at(r).int_value < temp_hash_reference_count)
                        {
                            has_diffuse_texture = true;

                            diffuse_texture_hash = rpkgs.at(mati_rpkg_index).hash.at(mati_hash_index).hash_reference_data.hash_reference.at(mati_properties.at(r).int_value);
                        }
                    }
                }
            }
        ...
        if (!has_normal_texture)
        {
            if (mati_properties.at(p).string_value == "mapTexture2DNormal_01")
            {
                for (uint32_t r = 0; r < mati_properties.size(); r++)
                {
                    ...
                    {
                        if (log_output)
                            std::cout << "Normal texture found at index: " << mati_properties.at(r).int_value << std::endl;

                        if (mati_properties.at(r).int_value < temp_hash_reference_count)
                        {
                            has_normal_texture = true;

                            normal_texture_hash = rpkgs.at(mati_rpkg_index).hash.at(mati_hash_index).hash_reference_data.hash_reference.at(mati_properties.at(r).int_value);
                        }
                    }
                }
            }
...
void mati::generate_json(std::string output_path)
{
    ...
    json["$schema"] = "https://wiki.notex.app/schemas/mati.schema.json";
    json["TYPE"] = type;
...
nlohmann::ordered_json mati::read_properties_json(nlohmann::ordered_json temp_json, uint32_t position, uint32_t parent, bool from_mati_file)
{
    uint8_t bytes1 = 0;
    uint32_t bytes4 = 0;
    uint64_t bytes8 = 0;
...
mati::mati(std::string mati_path, std::string mati_meta_path, uint64_t hash_value, std::string output_path, bool output_path_is_file)
{
    ...
    {
        file::create_directories(output_path);
        json_file = std::ofstream(file::output_path_append(util::uint64_t_to_hex_string(hash_value) + ".MATI.JSON", output_path), std::ofstream::binary);
    }
...
mati::mati(std::string json_path, uint64_t hash_value, std::string output_path, bool output_path_is_file)
{
    ...
    try
    {
        ...
        {
            file::create_directories(output_path);
            mati_file = std::ofstream(file::output_path_append(util::uint64_t_to_hex_string(hash_value) + ".MATI", output_path), std::ofstream::binary);
            meta_file = std::ofstream(file::output_path_append(util::uint64_t_to_hex_string(hash_value) + ".MATI.meta", output_path), std::ofstream::binary);
        }
