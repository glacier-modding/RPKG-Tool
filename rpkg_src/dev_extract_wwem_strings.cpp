                    ...
                    for (uint64_t a = 0; a < rpkgs.size(); a++)
                    {
                        for (uint64_t b = 0; b < rpkgs.at(a).hash_resource_types.size(); b++)
                        {
                            ...
                            for (uint64_t c = 0; c < rpkgs.at(a).hashes_indexes_based_on_resource_types.at(b).size(); c++)
                            {
                                ...
                                for (uint32_t k = 0; k < hash_reference_count; k++)
                                {
                                    if (rpkgs.at(a).hash.at(hash_index2).hash_reference_data.hash_reference.at(k) != rpkgs.at(i).hash.at(hash_index).hash_value || found_wwev)
                                        continue;
                                    ...
                                    if (rpkgs.at(a).hash.at(hash_index2).data.lz4ed)
                                    {
                                        LZ4_decompress_safe(input_data2.data(), output_data2.data(), static_cast<int>(hash_size), decompressed_size);
