...
void rpkg_function::hash_depends(std::string& input_path, std::string& filter)
{
    ...
    for (auto& filter : filters)
    {
        ...
        for (auto& i : hash_depends_data)
        {
            if (!i.hash_dependency.empty())
            {
                LOG(filter << " depends on " << i.hash_dependency_file_name.size() << " other hash files/resources in RPKG file: " <<
                    i.rpkg_file_name);
        ...
        for (auto& rpkg : rpkgs)
        {
            for (uint64_t j = 0; j < rpkg.hash.size(); j++)
            {
                ...
                for (uint64_t k = 0; k < temp_hash_reference_count; k++)
                {
                    ...
                    std::string rd = util::uint64_t_to_hex_string(rpkg.hash.at(j).hash_value) + "." + rpkg.hash.at(j).hash_resource_type;
