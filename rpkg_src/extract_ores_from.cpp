...
void rpkg_function::extract_ores_from(std::string& input_path, std::string& filter, std::string& output_path)
{
    ...
    uint32_t rpkg_index = get_latest_hash(temp_hash_value);
                ...
                if (!extract_single_hash || (extract_single_hash && filter == hash_string))
                {
                    for (uint64_t i = 0; i < rpkgs.size(); i++)
                    {
                        ...
                        if (it != rpkgs.at(i).hash_map.end())
                        {
                            ...
                            {
                                std::size_t found_position_hash = rpkgs.at(rpkg_index2).hash.at(hash_index2).
                                                                        hash_file_name.find(filters.at(z));

                                std::size_t found_position_ores = util::to_upper_case(ores_ioi_path).
                                    find(filters.at(z));

                                if ((found_position_hash != std::string::npos && filters.at(z) != "") || (
                                    found_position_ores != std::string::npos && filters.at(z) != ""))
                                {
                                    found = true;
