...
void rpkg_function::extract_to_rt_json(std::string& input_path, std::string& filter, std::string& version,
    ...
    if (version != "HM2016" && version != "HM2" && version != "HM3") {
        LOG_AND_RETURN("Error: Version (HM2016, HM2, or HM3) must be passed via -version on the command line.");
    }
    ...
    for (const auto& filter : filters) {
        ...
        for (uint64_t i = 0; i < rpkgs.size(); i++) {
            ...
            if (version == "HM2016") {
                if (std::find(hm2016_valid_resources.begin(), hm2016_valid_resources.end(),
                              rpkgs.at(rpkg_index).hash.at(it->second).hash_resource_type) ==
                    hm2016_valid_resources.end()) {
                    LOG_AND_RETURN("Error: " + rpkgs.at(rpkg_index).hash.at(it->second).hash_resource_type +
                                   " is not a valid resource type for ResourceLib for HM2016.");
                }
            } else if (version == "HM2") {
                if (std::find(hm2_valid_resources.begin(), hm2_valid_resources.end(),
                              rpkgs.at(rpkg_index).hash.at(it->second).hash_resource_type) ==
                    hm2_valid_resources.end()) {
                    LOG_AND_RETURN("Error: " + rpkgs.at(rpkg_index).hash.at(it->second).hash_resource_type +
                                   " is not a valid resource type for ResourceLib for HM2.");
                }
            } else if (version == "HM3") {
                if (std::find(hm3_valid_resources.begin(), hm3_valid_resources.end(),
                              rpkgs.at(rpkg_index).hash.at(it->second).hash_resource_type) ==
                    hm3_valid_resources.end()) {
                    LOG_AND_RETURN("Error: " + rpkgs.at(rpkg_index).hash.at(it->second).hash_resource_type +
                                   " is not a valid resource type for ResourceLib for HM3.");
                }
            }
            ...
            if (rpkgs.at(rpkg_index).hash.at(it->second).data.xored) {
                crypto::xor_data(temp_input_data.data(), static_cast<uint32_t>(temp_hash_size));
            }
            ...
            if (rpkgs.at(rpkg_index).hash.at(it->second).data.lz4ed) {
                LZ4_decompress_safe(temp_input_data.data(), temp_output_data.data(), static_cast<int>(temp_hash_size),
            ...
            if (version == "HM2016") {
                resource_tool_converter = HM2016_GetConverterForResource(
                        rpkgs.at(rpkg_index).hash.at(it->second).hash_resource_type.c_str());
            } else if (version == "HM2") {
                resource_tool_converter = HM2_GetConverterForResource(
                        rpkgs.at(rpkg_index).hash.at(it->second).hash_resource_type.c_str());
            } else if (version == "HM3") {
                resource_tool_converter = HM3_GetConverterForResource(
                        rpkgs.at(rpkg_index).hash.at(it->second).hash_resource_type.c_str());
            }

            JsonString* temp_json_input = nullptr;

            temp_json_input = resource_tool_converter->FromMemoryToJsonString(
                    static_cast<const void*>(temp_data.data()), (size_t) temp_data.size());
            ...
            std::ofstream file_temp = std::ofstream(file::output_path_append(
                                                            util::uint64_t_to_hex_string(rpkgs.at(rpkg_index).hash.at(it->second).hash_value) + "." +
                                                            rpkgs.at(rpkg_index).hash.at(it->second).hash_resource_type + ".JSON", output_path),
                                                    std::ofstream::binary);
