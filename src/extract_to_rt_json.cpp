...
void rpkg_function::extract_to_rt_json(std::string& input_path, std::string& filter, std::string& version, std::string& output_path)
{
    ...
    for (auto& filter : filters)
    {
        ...
        for (uint64_t i = 0; i < rpkgs.size(); i++)
        {
            ...
            if (rpkgs.at(rpkg_index).hash.at(it->second).data.lz4ed)
            {
                LZ4_decompress_safe(temp_input_data.data(), temp_output_data.data(), static_cast<int>(temp_hash_size), temp_decompressed_size);
            ...
            temp_json_input = resource_tool_converter->FromMemoryToJsonString(static_cast<const void*>(temp_data.data()), (size_t)temp_data.size());
