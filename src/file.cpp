...
uint64_t file::get_hash_value_from_path(const std::filesystem::path& path, const std::string& extension) {
    ...
    if (util::to_upper_case(file_name.substr((file_name.length() - extension.length()), extension.length())) !=
        util::to_upper_case(extension)) {
        return hash_value;
    }
...
bool file::write_to_file(const std::string& file_name, const char* data) {
    std::ofstream output_file = std::ofstream(file_name, std::ofstream::binary);
    ...
    output_file.write(data, std::strlen(data));
...
void file::parse_input_file_name(const std::string& input, const std::string& ends_with) {
    ...
    if (util::to_upper_case(input_file_name.substr((input_file_name.length() - ends_with_length), ends_with_length)) !=
        util::to_upper_case(ends_with)) {
...
bool file::has_extension(const std::string& input, const std::string& extension) {
    ...
    return util::to_upper_case(input.substr(input.length() - extension.length(), extension.length())) ==
           util::to_upper_case(extension);
}
