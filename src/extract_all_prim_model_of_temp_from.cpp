...
void rpkg_function::extract_all_prim_model_of_temp_from(std::string& input_path, const std::string& filter,
            ...
            if (it != rpkgs.at(rpkg_index).hash_map.end()) {
                ...
                if (rpkgs.at(rpkg_index).hash.at(it->second).hash_resource_type == "TEMP") {
                    ...
                    for (uint64_t j = 0; j < temp_temp.prim_depends_file_name.size(); j++) {
                        rpkg_function::extract_prim_model_from(rpkgs.at(temp_temp.prim_depends_rpkg_index.at(j).at(
                                                                       temp_temp.prim_depends_rpkg_index_index.at(j))).rpkg_file_path,
                                                               temp_temp.prim_depends_file_name.at(j), output_path);
                    }
                }
            }
        }
