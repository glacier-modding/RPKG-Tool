...
void rpkg_function::recursive_temp_loader(uint32_t rpkgs_index, const uint32_t hash_index, const uint32_t temp_version,
                                          std::unordered_map<uint32_t, uint32_t>& parents_map,
                                          std::vector<uint32_t> temps_indexes, const uint32_t level,
                                          const uint32_t level_max, const uint32_t temps_max) {
    ...
    if (it != parents_map.end()) {
        LOG("Recursive loop detected at level " + util::uint32_t_to_string(level) +
            " and the duplicate TEMP file is: " +
            util::uint64_t_to_hex_string(rpkgs.at(rpkgs_index).hash.at(hash_index).hash_value) + "." +
        ...
        for (uint64_t i = 0; i < temps_indexes.size(); i++) {
            LOG("Parent temps index(" + util::uint32_t_to_string(i) + "): " +
                util::uint32_t_to_string(temps_indexes.at(i)) + ", TEMP File Name: " +
                temps.at(temps_indexes.at(i)).temp_file_name);
        }
        return;
    }
    ...
    for (uint64_t i = 0; i < temps.at(temps_index).temp_depends_file_name.size(); i++) {
        ...
        recursive_temp_loader(temps.at(temps_index).temp_depends_rpkg_index.at(i).at(
                                      temps.at(temps_index).temp_depends_rpkg_index_index.at(i)),
                              temps.at(temps_index).temp_depends_hash_index.at(i).at(
                                      temps.at(temps_index).temp_depends_hash_index_index.at(i)), temp_version,
                              parents_map, temps_indexes, level + 1, level_max, temps_max);
