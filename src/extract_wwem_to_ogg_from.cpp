        ...
        if (rpkg.rpkg_file_path == input_path || !input_path_is_rpkg_file) {
            for (uint64_t r = 0; r < rpkg.hash_resource_types.size(); r++) {
                ...
                for (uint64_t j = 0; j < rpkg.hashes_indexes_based_on_resource_types.at(r).size(); j++) {
                    ...
                    if (((wwem_count_current * static_cast<uint64_t>(100000)) / wwem_count) %
                        static_cast<uint64_t>(10) == 0 && wwem_count_current > 0) {
                        stringstream_length = console::update_console(message, wwem_hash_size_total,
                                                                      wwem_hash_size_current, start_time,
                                                                      stringstream_length);
                    }
                    ...
                    if (it != hash_list_hash_map.end()) {
                        ...
                        wwem_ioi_path = file::output_path_append("WWEM\\" + rpkg.rpkg_file_name + "\\assembly",
                                                                 output_path);
                        ...
                        if (pos1 != std::string::npos && pos2 != std::string::npos && pos3 != std::string::npos &&
                            (pos2 - pos1) > (string_length + 2)) {
                            wwem_ioi_path.append(
                                    hash_list_string.substr((pos1 + string_length), pos2 - (pos1 + string_length)));
