...
void rpkg_function::extract_all_text_from(std::string& input_path, std::string& filter, std::string& output_path)
{
    ...
    for (auto& rpkg : rpkgs)
    {
        ...
        {
            if (rpkg.hash_resource_types.at(r) != "TEXT")
                continue;
            ...
            for (uint64_t j2 = 0; j2 < rpkg.hashes_indexes_based_on_resource_types.at(r).size(); j2++)
            {
                ...
                std::string hashFileName = util::uint64_t_to_hex_string(rpkg.hash.at(hashIndex).hash_value) + "." +
        ...
        if (rpkg.rpkg_file_path == input_path || !input_path_is_rpkg_file)
        {
            for (uint64_t r = 0; r < rpkg.hash_resource_types.size(); r++)
            {
                ...
                for (uint64_t j = 0; j < rpkg.hashes_indexes_based_on_resource_types.at(r).size(); j++)
                {
                    ...
                    if (((prim_count_current * (uint64_t) 100000) / (uint64_t) prim_count) % (uint64_t) 10 == 0 &&
                        prim_count_current > 0)
                    {
                            ...
                            rpkg.hash.at(hash_index).hash_value)))
                    {
                        ...
                        for (uint64_t z1 = 0; z1 < filters.size(); z1++)
                        {
                            ...
                            if ((found_position_hash != std::string::npos && filters.at(z1) != ""))
                            {
                                found = true;
                        ...
                        if (found || filter == "")
                        {
                            ...
                            rpkg_function::extract_text_from(rpkg.rpkg_file_path, util::uint64_t_to_hex_string(
                                    rpkg.hash.at(hash_index).hash_value), output_path);
                        }
                    }
                }
            }
