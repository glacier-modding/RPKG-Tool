...
void gltf::output_to_single_file(asset3ds& asset3ds_data, std::string& file_path, int type, bool rotate) {
    ...
    for (uint32_t o = 0; o < asset3ds_data.vertexes.size(); o++) {
        ...
        meshPrimitive.indicesAccessorId = bufferBuilder.AddAccessor(asset3ds_data.indexes.at(o),
                                                                    {TYPE_SCALAR, COMPONENT_UNSIGNED_SHORT}).id;

        bufferBuilder.AddBufferView(BufferViewTarget::ARRAY_BUFFER);
        meshPrimitive.attributes["POSITION"] = bufferBuilder.AddAccessor(asset3ds_data.vertexes.at(o),
                                                                         {TYPE_VEC3, COMPONENT_FLOAT, false,
                                                                          std::move(vertexes_min.at(o)),
                                                                          std::move(vertexes_max.at(o))}).id;

        bufferBuilder.AddBufferView(BufferViewTarget::ARRAY_BUFFER);
        meshPrimitive.attributes["NORMAL"] = bufferBuilder.AddAccessor(asset3ds_data.normals.at(o),
                                                                       {TYPE_VEC3, COMPONENT_FLOAT}).id;

        //bufferBuilder.AddBufferView(BufferViewTarget::ARRAY_BUFFER);
        //meshPrimitive.attributes["TANGENT"] = bufferBuilder.AddAccessor(asset3ds_data.tangents.at(o), { TYPE_VEC4, COMPONENT_FLOAT }).id;

        bufferBuilder.AddBufferView(BufferViewTarget::ARRAY_BUFFER);
        meshPrimitive.attributes["TEXCOORD_0"] = bufferBuilder.AddAccessor(asset3ds_data.uvs.at(o),
                                                                           {TYPE_VEC2, COMPONENT_FLOAT}).id;

        if (asset3ds_data.colors.at(o).size() > 0) {
            bufferBuilder.AddBufferView(BufferViewTarget::ARRAY_BUFFER);
            meshPrimitive.attributes["COLOR_0"] = bufferBuilder.AddAccessor(asset3ds_data.colors.at(o),
                                                                            {TYPE_VEC4, COMPONENT_UNSIGNED_BYTE,
                                                                             true}).id;
        }
        ...
        if (asset3ds_data.weighted.at(o)) {
            ...
            meshPrimitive.attributes["JOINTS_0"] = bufferBuilder.AddAccessor(
                    asset3ds_data.vertexes_weighted_bone_ids_0.at(o), {TYPE_VEC4, COMPONENT_UNSIGNED_BYTE}).id;

            bufferBuilder.AddBufferView(BufferViewTarget::ARRAY_BUFFER);
            meshPrimitive.attributes["JOINTS_1"] = bufferBuilder.AddAccessor(
                    asset3ds_data.vertexes_weighted_bone_ids_1.at(o), {TYPE_VEC4, COMPONENT_UNSIGNED_BYTE}).id;

            bufferBuilder.AddBufferView(BufferViewTarget::ARRAY_BUFFER);
            meshPrimitive.attributes["WEIGHTS_0"] = bufferBuilder.AddAccessor(
                    asset3ds_data.vertexes_weighted_weights_0.at(o), {TYPE_VEC4, COMPONENT_FLOAT}).id;

            bufferBuilder.AddBufferView(BufferViewTarget::ARRAY_BUFFER);
            meshPrimitive.attributes["WEIGHTS_1"] = bufferBuilder.AddAccessor(
                    asset3ds_data.vertexes_weighted_weights_1.at(o), {TYPE_VEC4, COMPONENT_FLOAT}).id;
            ...
            if (!bones_added) {
                ...
                for (uint32_t b = 0; b < asset3ds_data.bones_data.size(); b++) {
                    ...
                    bones.back().translation = Vector3(asset3ds_data.bones_positions.at(b * 8 + 4),
                                                       asset3ds_data.bones_positions.at(b * 8 + 5),
                                                       asset3ds_data.bones_positions.at(b * 8 + 6));
                    bones.back().rotation = Quaternion(asset3ds_data.bones_positions.at(b * 8),
                                                       asset3ds_data.bones_positions.at(b * 8 + 1),
                                                       asset3ds_data.bones_positions.at(b * 8 + 2),
                                                       asset3ds_data.bones_positions.at(b * 8 + 3));
                }
                ...
                temp_skin.inverseBindMatricesAccessorId = bufferBuilder.AddAccessor(
                        asset3ds_data.bones_inverse_matrices, {TYPE_MAT4, COMPONENT_FLOAT}).id;
...
void gltf::output_to_single_file_with_textures(asset3ds& asset3ds_data, std::string& file_path, int type, bool rotate,
                                               std::vector<jpg_textures> textures) {
    std::vector<std::vector<float>> vertexes_min;
    std::vector<std::vector<float>> vertexes_max;
    ...
    for (auto& t : textures) {
        ...
        if (t.has_diffuse_texture) {
            Image image;
            image.bufferViewId = bufferBuilder.AddBufferView(t.texture_diffuse_jpg.data(),
                                                             t.texture_diffuse_jpg.size()).id;
        ...
        if (t.has_normal_texture) {
            Image image;
            image.bufferViewId = bufferBuilder.AddBufferView(t.texture_normal_jpg.data(),
                                                             t.texture_normal_jpg.size()).id;
            image.mimeType = "image/jpeg";
            std::string image_id = document.images.Append(std::move(image), AppendIdPolicy::GenerateOnEmpty).id;
        ...
        if (t.has_emissive_texture) {
            Image image;
            image.bufferViewId = bufferBuilder.AddBufferView(t.texture_emissive_jpg.data(),
                                                             t.texture_emissive_jpg.size()).id;
            image.mimeType = "image/jpeg";
            std::string image_id = document.images.Append(std::move(image), AppendIdPolicy::GenerateOnEmpty).id;
        ...
        material_ids[t.material_id] = document.materials.Append(std::move(material),
                                                                AppendIdPolicy::GenerateOnEmpty).id;
    }
