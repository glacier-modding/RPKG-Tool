...
bool rpkg_function::import_rpkg_meta(rpkg& rpkg_meta_data, const std::string& rpkg_meta_file_path,
    ...
    if (rpkg_meta_data.rpkg_file_version == 1 &&
        (static_cast<uint64_t>(rpkg_meta_data.header.patch_count) * static_cast<uint64_t>(0x8) +
         static_cast<uint64_t>(0x14) + static_cast<uint64_t>(0x10)) >= rpkg_meta_file_size) {
        rpkg_meta_data.is_patch_file = false;

        LOG("RPKGv1 file " << rpkg_meta_file_path << " is not a patch file.");
    } else if (rpkg_meta_data.rpkg_file_version == 2 &&
               (static_cast<uint64_t>(rpkg_meta_data.header.patch_count) * static_cast<uint64_t>(0x8) +
                static_cast<uint64_t>(0x1D) + static_cast<uint64_t>(0x10)) >= rpkg_meta_file_size) {
        rpkg_meta_data.is_patch_file = false;
        ...
        if (rpkg_meta_data.rpkg_file_version == 1) {
            rpkg_meta_file.seekg(
                    (static_cast<uint64_t>(rpkg_meta_data.header.patch_count) * static_cast<uint64_t>(0x8) +
                     static_cast<uint64_t>(0x14)), std::ifstream::beg);
        } else {
            rpkg_meta_file.seekg(
                    (static_cast<uint64_t>(rpkg_meta_data.header.patch_count) * static_cast<uint64_t>(0x8) +
                     static_cast<uint64_t>(0x1D)), std::ifstream::beg);
        }
        ...
        if (rpkg_meta_data.rpkg_file_version == 1 && patchValue ==
                                                     (static_cast<uint64_t>(rpkg_meta_data.header.hash_header_table_size) +
                                                      static_cast<uint64_t>(rpkg_meta_data.header.hash_resource_table_size) +
                                                      static_cast<uint64_t>(rpkg_meta_data.header.patch_count) *
                                                      static_cast<uint64_t>(0x8) + static_cast<uint64_t>(0x14)) &&
            patchZeroTest == 0x0) {
            rpkg_meta_data.is_patch_file = true;

            LOG("RPKGv1 file " << rpkg_meta_file_path << " is a patch file.");
        } else if (rpkg_meta_data.rpkg_file_version == 2 && patchValue ==
                                                            (static_cast<uint64_t>(rpkg_meta_data.header.hash_header_table_size) +
                                                             static_cast<uint64_t>(rpkg_meta_data.header.hash_resource_table_size) +
                                                             static_cast<uint64_t>(rpkg_meta_data.header.patch_count) *
