    ...
    if (hash_list_loaded) {
        ...
        if (it2 != hash_list_hash_map.end()) {
            if (hash_list_hash_strings.at(it2->second) != "") {
                if (hash_value ==
                    std::strtoull(generic_function::compute_ioi_hash(hash_list_hash_strings.at(it2->second)).c_str(),
                                  nullptr, 16)) {
...
std::wstring util::string_to_wstring(const std::string& input_string) {
    ...
    if (num_chars) {
        ...
        if (MultiByteToWideChar(CP_ACP, MB_ERR_INVALID_CHARS, input_string.c_str(), input_string.length(), &wstrTo[0],
                                num_chars)) {
            return wstrTo;
        }
    }
