...
text::text(uint64_t rpkgs_index, uint64_t hash_index)
{
    ...
    {
        temp_texd.load(texd_depends_rpkg_index.at(0).at(texd_depends_rpkg_index_index.at(0)), texd_depends_hash_index.at(0).at(texd_depends_hash_index_index.at(0)));
    ...
    if (text_mips_data_1.at(0) != text_mips_data_2.at(0))//(!texd_found && text_mips_data_1.at(0) != text_mips_data_2.at(0))
    {
        text_is_lz4ed = true;
...
void text::generate_tga_meta_files(const std::string& meta_path) const
{
    ...
    if (!output_file.good())
    {
        LOG_AND_EXIT("Error: TEXT TGA meta file " + meta_path + "\\" + text_file_name + ".tga.meta" + " could not be created.");
    }
    ...
    if (texd_found)
    {
        ...
        rpkg_function::extract_hash_meta(texd_depends_rpkg_index.at(0).at(texd_depends_rpkg_index_index.at(0)), texd_depends_hash_index.at(0).at(texd_depends_hash_index_index.at(0)), meta_file_path);
    }
}
