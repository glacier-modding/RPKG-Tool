...
public:
	...
	mati(std::string mati_path, std::string mati_meta_path, uint64_t hash_value, std::string output_path, bool output_path_is_file);

	void load_hash_depends();
	void map_textures();
	void read_properties(uint32_t position, uint32_t parent);
	void generate_json(std::string output_path);
	nlohmann::ordered_json read_properties_json(nlohmann::ordered_json json, uint32_t position, uint32_t parent, bool from_mati_file);
