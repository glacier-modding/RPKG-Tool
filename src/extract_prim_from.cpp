    ...
    for (auto& filter : filters) {
        ...
        for (uint64_t i = 0; i < rpkgs.size(); i++) {
            ...
            if (type == GLB_SINGLE) {
                ...
                if (output_path.empty()) {
                    asset_file_name = std::filesystem::current_path().generic_string() + "/" +
                                      util::uint64_t_to_hex_string(
                                              rpkgs.at(rpkg_index).hash.at(it->second).hash_value) + "." +
                ...
                std::string asset_file_name = file::output_path_append(
                        util::uint64_t_to_hex_string(rpkgs.at(rpkg_index).hash.at(it->second).hash_value) + "." +
                        rpkgs.at(rpkg_index).hash.at(it->second).hash_resource_type + ".gltf", output_path);
                ...
                std::string asset_file_name = file::output_path_append(
                        util::uint64_t_to_hex_string(rpkgs.at(rpkg_index).hash.at(it->second).hash_value) + "." +
                        rpkgs.at(rpkg_index).hash.at(it->second).hash_resource_type + ".obj", output_path);
