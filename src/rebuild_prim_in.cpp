            ...
            for (uint32_t m = 0; m < prim_object_to_mesh_index.size(); m++)
            {
                if (!prim_object_to_mesh_index.at(m).empty())
                {
                    ...
                    for (const auto& meshPrimitive : temp_mesh.primitives)
                    {
                        ...
                        {
                            if (meshPrimitive.HasAttribute("JOINTS_0") && meshPrimitive.HasAttribute("WEIGHTS_0") && !temp_node.skinId.empty())
                            {
                                ...
                                if (meshPrimitive.HasAttribute("JOINTS_1") && meshPrimitive.HasAttribute("WEIGHTS_1"))
                                {
                                    ...
                                    if (static_cast<uint32_t>(joints_0_data.size() / 4) != vertex_count || static_cast<uint32_t>(joints_1_data.size() / 4) != vertex_count || static_cast<uint32_t>(weights_0_data.size() / 4) != vertex_count || static_cast<uint32_t>(weights_1_data.size() / 4) != vertex_count)
                                    {
                                ...
                                {
                                    if (static_cast<uint32_t>(joints_0_data.size() / 4) != vertex_count || static_cast<uint32_t>(weights_0_data.size() / 4) != vertex_count)
                                    {
                                        LOG("Error: " + glb_file_names.at(j) + "'s mesh " + temp_mesh.name + " weighted data does not conform to IOI's specs (per vertex): JOINTS_0 (4 values), WEIGHTS_0 (4 values), JOINTS_1 (2 values), WEIGHTS_1 (2 values).");
                        ...
                        if (meshPrimitive.TryGetAttributeAccessorId("NORMAL", accessorId))
                        {
                            ...
                            if (static_cast<uint32_t>(normals_data.size() / 3) != vertex_count)
                            {
                                ...
                                task_status_string = "Error: " + glb_file_names.at(j) + "'s mesh " + temp_mesh.name + " vertex float size (" + util::uint32_t_to_string(vertex_count) + ") not equal to normal float size (" + util::uint32_t_to_string(static_cast<uint32_t>(normals_data.size() / 3)) + ").";
                        ...
                        if (meshPrimitive.TryGetAttributeAccessorId("TEXCOORD_0", accessorId))
                        {
                            ...
                            if (static_cast<uint32_t>(uvs_data.size() / 2) != vertex_count)
                            {
                                ...
                                task_status_string = "Error: " + glb_file_names.at(j) + "'s mesh " + temp_mesh.name + " vertex float size (" + util::uint32_t_to_string(vertex_count) + ") not equal to TEXTCOORD_0 float size (" + util::uint32_t_to_string(static_cast<uint32_t>(uvs_data.size() / 2)) + ").";
                        ...
                        {
                            if (temp_prim.prim_weighted_meshes.at(m).prim_mesh_instance.prim_object_instance.sub_type == static_cast<uint32_t>(prim::prim_object::SUBTYPE::WEIGHTED) || ((temp_prim.prim_weighted_sub_meshes.at(m).prim_object_instance.property_flags & static_cast<uint32_t>(prim::prim_object::PROPERTY_FLAGS::COLOR1)) == 0x0))
                            {
                                if (meshPrimitive.TryGetAttributeAccessorId("COLOR_0", accessorId))
                                {
                                    ...
                                    if (accessor.type == TYPE_VEC4 && accessor.componentType == COMPONENT_UNSIGNED_BYTE)
                                    {
                                        ...
                                        if (static_cast<uint32_t>(colors_data_uint8_t.size() / 4) != vertex_count)
                                        {
                                            ...
                                            task_status_string = "Error: " + glb_file_names.at(j) + "'s mesh " + temp_mesh.name + " color size (" + util::uint32_t_to_string(static_cast<uint32_t>(colors_data_uint8_t.size() / 4)) + ") not equal to vertex size (" + util::uint32_t_to_string(vertex_count) + ").";
                                    ...
                                    else if (accessor.type == TYPE_VEC4 && accessor.componentType == COMPONENT_UNSIGNED_SHORT)
                                    {
                                        ...
                                        if (static_cast<uint32_t>(colors_data_uint16_t.size() / 4) != vertex_count)
                                        {
                                            ...
                                            task_status_string = "Error: " + glb_file_names.at(j) + "'s mesh " + temp_mesh.name + " color size (" + util::uint32_t_to_string(static_cast<uint32_t>(colors_data_uint16_t.size() / 4)) + ") not equal to vertex size (" + util::uint32_t_to_string(vertex_count) + ").";
                                        ...
                                        for (uint32_t v = 0; v < vertex_count; v++)
                                        {
                                            ...
                                            uint8_t temp_uint8_t = std::round((static_cast<double>(colors_data_uint16_t[x]) / static_cast<double>(0xFFFF)) * static_cast<double>(0xFF));

                                            std::memcpy(&char1, &temp_uint8_t, sizeof(uint8_t));
                                            prim_file_data.push_back(char1);

                                            temp_uint8_t = std::round((double)((colors_data_uint16_t[y]) / static_cast<double>(0xFFFF)) * static_cast<double>(0xFF));

                                            std::memcpy(&char1, &temp_uint8_t, sizeof(uint8_t));
                                            prim_file_data.push_back(char1);

                                            temp_uint8_t = std::round((double)((colors_data_uint16_t[z]) / static_cast<double>(0xFFFF)) * static_cast<double>(0xFF));

                                            std::memcpy(&char1, &temp_uint8_t, sizeof(uint8_t));
                                            prim_file_data.push_back(char1);

                                            temp_uint8_t = std::round((double)((colors_data_uint16_t[w]) / static_cast<double>(0xFFFF)) * static_cast<double>(0xFF));
                        ...
                        {
                            if (temp_prim.prim_meshes.at(m).prim_object_instance.sub_type == static_cast<uint32_t>(prim::prim_object::SUBTYPE::WEIGHTED) || ((temp_prim.prim_sub_meshes.at(m).prim_object_instance.property_flags & static_cast<uint32_t>(prim::prim_object::PROPERTY_FLAGS::COLOR1)) == 0x0))
                            {
                                if (meshPrimitive.TryGetAttributeAccessorId("COLOR_0", accessorId))
                                {
                                    const Accessor& accessor = document.accessors.Get(accessorId);
                                    ...
                                    else if (accessor.type == TYPE_VEC4 && accessor.componentType == COMPONENT_UNSIGNED_SHORT)
                                    {
                                        ...
                                        for (uint32_t v = 0; v < vertex_count; v++)
                                        {
                                            ...
                                            temp_uint8_t = std::round(((colors_data_uint16_t[y]) / static_cast<double>(0xFFFF)) * static_cast<double>(0xFF));

                                            std::memcpy(&char1, &temp_uint8_t, sizeof(uint8_t));
                                            prim_file_data.push_back(char1);

                                            temp_uint8_t = std::round(((colors_data_uint16_t[z]) / static_cast<double>(0xFFFF)) * static_cast<double>(0xFF));

                                            std::memcpy(&char1, &temp_uint8_t, sizeof(uint8_t));
                                            prim_file_data.push_back(char1);

                                            temp_uint8_t = std::round(((colors_data_uint16_t[w]) / static_cast<double>(0xFFFF)) * static_cast<double>(0xFF));
