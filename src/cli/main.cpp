...
#error "This platform is not yet supported"
    ...
    if (argc < 2) {
        ...
        if (std::string(argv[1]) == "-i") {
            ...
            std::cout << "Type q or quit to exit rpkg-cli, and type h or help to display usage info." << std::endl
                      << std::endl;
        ...
        if (command_line::all_args_are_paths(argc, argv)) {
            std::vector<std::string> dragged_and_dropped_files = command_line::get_dragged_and_dropped_files(argc,
                                                                                                             argv);
