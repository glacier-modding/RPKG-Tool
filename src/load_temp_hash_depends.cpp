...
int rpkg_function::load_temp_hash_depends(uint64_t rpkg_index, uint64_t hash_index) {
    ...
    temporary_hash_depends_data.hash_string = util::uint64_t_to_hex_string(
            rpkgs.at(rpkg_index).hash.at(hash_index).hash_value);

    std::string hash_string = temporary_hash_depends_data.hash_string;

    uint32_t temp_hash_reference_count =
            rpkgs.at(rpkg_index).hash.at(hash_index).hash_reference_data.hash_reference_count & 0x3FFFFFFF;

    LOG(hash_string + " has " + util::uint32_t_to_string(temp_hash_reference_count) + " dependencies in " +
        rpkgs.at(rpkg_index).rpkg_file_name);
    ...
    if (temp_hash_reference_count > 0) {
        for (uint64_t y = 0; y < temp_hash_reference_count; y++) {
            ...
            for (uint64_t z = 0; z < rpkgs.size(); z++) {
                std::unordered_map<uint64_t, uint64_t>::iterator it3 = rpkgs.at(z).hash_map.find(
                        rpkgs.at(rpkg_index).hash.at(hash_index).hash_reference_data.hash_reference.at(y));

                if (it3 != rpkgs.at(z).hash_map.end()) {
                    LOG(rpkgs.at(z).hash.at(it3->second).hash_resource_type);

                    if (!found) {
                        temporary_hash_depends_data.hash_dependency_file_name.push_back(
                                rpkgs.at(z).hash.at(it3->second).hash_file_name);
                    }
            ...
            if (!found) {
                temporary_hash_depends_data.hash_dependency_file_name.push_back(
                        rpkgs.at(rpkg_index).hash.at(hash_index).hash_reference_data.hash_reference_string.at(y));
            }

            std::string ioi_string = "";

            std::unordered_map<uint64_t, uint64_t>::iterator it2 = hash_list_hash_map.find(
                    std::strtoull(temporary_hash_depends_data.hash_dependency_file_name.back().c_str(), nullptr, 16));

            if (it2 != hash_list_hash_map.end()) {
                temporary_hash_depends_data.hash_dependency_file_name.back() = util::to_upper_case(
                        hash_list_hash_file_names.at(it2->second));
            ...
            temporary_hash_depends_data.hash_dependency.push_back(
                    util::uint64_t_to_hex_string(temporary_hash_depends_data.hash_value));
    ...
    LOG(hash_string + " has dependencies in " + util::uint32_t_to_string((uint32_t) rpkg_dependency_count) +
        " RPKG files:");
    ...
    for (uint64_t x = 0; x < temp_hash_depends_data.size(); x++) {
        if (temp_hash_depends_data.at(x).hash_dependency.size() > 0) {
            LOG(hash_string + " depends on " +
                util::uint32_t_to_string((uint32_t) temp_hash_depends_data.at(x).hash_dependency_file_name.size()) +
                " other hash files/resources in RPKG file: " + temp_hash_depends_data.at(x).rpkg_file_name);
            ...
            if (temp_hash_depends_data.at(x).hash_dependency_file_name.size() > 0) {
                ...
                for (uint64_t y = 0; y < temp_hash_depends_data.at(x).hash_dependency_file_name.size(); y++) {
                    ...
                    if (temp_hash_depends_data.at(x).hash_dependency_in_rpkg.at(y).size() > 0) {
                        if (temp_hash_depends_data.at(x).hash_dependency_file_name.at(y).substr(
                                (temp_hash_depends_data.at(x).hash_dependency_file_name.at(y).length() - 4), 4) ==
                            "TBLU") {
                            if (temp_tblu_depends_count == 0) {
                                tblu_file_name = temp_hash_depends_data.at(x).hash_dependency_file_name.at(y);
                                tblu_in_rpkg = temp_hash_depends_data.at(x).hash_dependency_in_rpkg.at(y).at(
                                        temp_hash_depends_data.at(x).hash_dependency_in_rpkg.at(y).size() - 1);
                            } else {
                        ...
                        for (uint64_t z = 0;
                             z < temp_hash_depends_data.at(x).hash_dependency_in_rpkg.at(y).size(); z++) {
