...
uint32_t rpkg_function::get_latest_hash(uint64_t hash_value) {
    ...
    for (uint64_t i = 0; i < rpkgs.size(); i++) {
        ...
        if (it2 != rpkgs.at(i).hash_map.end()) {
            ...
            std::string_view hash_in_rpkgs_string_view(hash_in_rpkgs_upper_case.c_str(),
                                                       hash_in_rpkgs_upper_case.length());
            ...
            if (pos_chunk != std::string::npos && pos_patch != std::string::npos) {
                ...
                chunk_numbers.push_back(
                        std::strtol(std::string(hash_in_rpkgs_string_view.substr(pos_chunk + 5, pos_patch)).c_str(),
                                    nullptr, 10));
                patch_numbers.push_back(
                        std::strtol(std::string(hash_in_rpkgs_string_view.substr(pos_patch + 5, pos_ext)).c_str(),
                                    nullptr, 10));
                ...
                chunk_numbers.push_back(
                        std::strtol(std::string(hash_in_rpkgs_string_view.substr(pos_chunk + 5)).c_str(), nullptr, 10));
                ...
                //std::cout << std::string(hash_in_rpkgs_string_view.substr(pos_patch + 5, pos_ext)) << std::endl;
                chunk_numbers.push_back(
                        std::strtol(std::string(hash_in_rpkgs_string_view.substr(pos_dlc + 3, pos_patch)).c_str(),
                                    nullptr, 10));
                patch_numbers.push_back(
                        std::strtol(std::string(hash_in_rpkgs_string_view.substr(pos_patch + 5, pos_ext)).c_str(),
                                    nullptr, 10));
                ...
                //std::cout << std::string(hash_in_rpkgs_string_view.substr(pos_chunk + 5, pos_ext)) << std::endl;
                chunk_numbers.push_back(
                        std::strtol(std::string(hash_in_rpkgs_string_view.substr(pos_dlc + 3)).c_str(), nullptr, 10));
                patch_numbers.push_back(0);
            ...
            if (log_output)
                    LOG(util::uint64_t_to_hex_string(rpkgs.at(i).hash.at(it2->second).hash_value) + "." +
                        rpkgs.at(i).hash.at(it2->second).hash_resource_type + " was found in RPKG file: " +
            ...
            if (found_in_patch_list) {
                ...
                if (pos_chunk != std::string::npos && pos_patch != std::string::npos) {
                    ...
                    //std::cout << std::string(hash_in_rpkgs_string_view.substr(pos_chunk + 5, pos_ext)) << std::endl;
                    chunk_numbers.push_back(
                            std::strtol(std::string(hash_in_rpkgs_string_view.substr(pos_chunk + 5)).c_str(), nullptr,
                                        10));
                    patch_numbers.push_back(0);
                    ...
                    //std::cout << std::string(hash_in_rpkgs_string_view.substr(pos_chunk + 5, pos_ext)) << std::endl;
                    chunk_numbers.push_back(
                            std::strtol(std::string(hash_in_rpkgs_string_view.substr(pos_dlc + 3)).c_str(), nullptr,
                                        10));
                    patch_numbers.push_back(0);
                ...
                if (log_output)
                        LOG(util::uint64_t_to_hex_string(hash_value) +
                            " was found in the patch deletion list of RPKG file: " + rpkgs.at(i).rpkg_file_name);
    ...
    for (uint64_t i = 0; i < hash_in_rpkgs.size(); i++) {
        for (uint64_t j = 0; j < chunk_numbers_deleted.size(); j++) {
            if (chunk_numbers.at(i) == chunk_numbers_deleted.at(j) &&
                patch_numbers.at(i) <= patch_numbers_deleted.at(j)) {
