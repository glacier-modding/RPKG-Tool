    ...
    for (uint64_t p = 0; p < json_file_paths.size(); p++) {
        ...
        if (file::path_exists(json_file_paths.at(p) + ".meta")) {
            ...
            for (const auto& it : input_json.items()) {
                ...
                if (!language_found) {
                    LOG_AND_EXIT("Error: JSON file " + json_file_paths.at(p) +
                                 " is malformed and can not be rebuilt into a DLGE file/resource.");
                }
            }
            ...
            if (category_language_count != (number_of_dlge_categories * number_of_languages)) {
                LOG_AND_EXIT(
                        "Error: Number of strings in the input DLGE JSON file do not match the number of strings in the meta file.");
            }
            ...
            for (uint64_t i = 0; i < number_of_dlge_categories; i++) {
                ...
                dlge_data.push_back(0x0);
                dlge_data.push_back(0x0);
                dlge_data.push_back(static_cast<char>(0xFF));
                dlge_data.push_back(static_cast<char>(0xFF));
                dlge_data.push_back(static_cast<char>(0xFF));
                dlge_data.push_back(static_cast<char>(0xFF));
                dlge_data.push_back(static_cast<char>(0xFF));
                ...
                for (const auto& it : input_json.items()) {
                    if (key_current >= key_range_min && key_current < key_range_max) {
                        ...
                        for (char& k : char4) {
                            dlge_data.push_back(k);
                        }

                        for (uint32_t k = 0; k < string_length / 8; k++) {
                            uint32_t data[2];
                            std::memcpy(data, &temp_string[static_cast<uint64_t>(k) * static_cast<uint64_t>(8)],
                                        sizeof(uint32_t));
                            std::memcpy(data + 1, &temp_string[static_cast<uint64_t>(k) * static_cast<uint64_t>(8) +
                                                               static_cast<uint64_t>(4)], sizeof(uint32_t));

                            crypto::xtea_encrypt_localization(data);

                            std::memcpy(&temp_string[static_cast<uint64_t>(k) * static_cast<uint64_t>(8)], data,
                                        sizeof(uint32_t));
                            std::memcpy(&temp_string[static_cast<uint64_t>(k) * static_cast<uint64_t>(8) +
