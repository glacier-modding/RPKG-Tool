            ...
            for (uint64_t i = 0; i < number_of_dlge_categories; i++)
            {
                ...
                for (const auto &it : input_json.items())
                {
                    if (key_current >= key_range_min && key_current < key_range_max)
                    {
                        ...
                        {
                            uint32_t data[2];
                            std::memcpy(data, &temp_string[static_cast<uint64_t>(k) * static_cast<uint64_t>(8)], sizeof(uint32_t));
                            std::memcpy(data + 1, &temp_string[static_cast<uint64_t>(k) * static_cast<uint64_t>(8) + static_cast<uint64_t>(4)], sizeof(uint32_t));

                            crypto::xtea_encrypt_localization(data);

                            std::memcpy(&temp_string[static_cast<uint64_t>(k) * static_cast<uint64_t>(8)], data, sizeof(uint32_t));
                            std::memcpy(&temp_string[static_cast<uint64_t>(k) * static_cast<uint64_t>(8) + static_cast<uint64_t>(4)], data + 1, sizeof(uint32_t));
                        }
