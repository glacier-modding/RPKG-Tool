                            ...
                            if (it2.value().contains("Language"))
                            {
                                if (it2.value()["Language"] == languages.at(i) && language_in_locr.at(i))
                                {
                                    ...
                                    for (const auto &it3 : it.value().items())
                                    {
                                        ...
                                        {
                                            temp_locr_language_section_string_hashes.push_back(it3.value()["StringHash"]);
                                            ...
                                            if (symKey)
                                            {
                                                locr_section_size += static_cast<uint32_t>(temp_string.size()) + static_cast<uint32_t>(0x9);
                                            }
                                            else
                                            {
                                            ...
                                            temp_locr_language_section_string_lengths.push_back(static_cast<uint32_t>(temp_string.size()));
                                        }
                                    }
                ...
                for (const auto &it : input_json.items())
                {
                    for (const auto &it2 : it.value().items())
                    {
                        if (it2.value().contains("Language"))
                        {
                            ...
                            {
                                uint32_t section_string_count = static_cast<uint32_t>(locr_language_section_string_hashes.at(i).size());
                                ...
                                for (uint64_t j = 0; j < locr_language_section_strings.at(i).size(); j++)
                                {
                                    ...
                                    if (symKey)
                                    {
                                        ...
                                        {
                                            locr_data.push_back(crypto::symmetric_key_encrypt_localization(locr_language_section_strings.at(i).at(j)[static_cast<uint64_t>(k)]));
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        {
                                            uint32_t data[2];
                                            std::memcpy(data, &locr_language_section_strings.at(i).at(j)[static_cast<uint64_t>(k) * static_cast<uint64_t>(8)], sizeof(uint32_t));
                                            std::memcpy(data + 1, &locr_language_section_strings.at(i).at(j)[static_cast<uint64_t>(k) * static_cast<uint64_t>(8) + static_cast<uint64_t>(4)], sizeof(uint32_t));

                                            crypto::xtea_encrypt_localization(data);

                                            std::memcpy(&locr_language_section_strings.at(i).at(j)[static_cast<uint64_t>(k) * static_cast<uint64_t>(8)], data, sizeof(uint32_t));
                                            std::memcpy(&locr_language_section_strings.at(i).at(j)[static_cast<uint64_t>(k) * static_cast<uint64_t>(8) + static_cast<uint64_t>(4)], data + 1, sizeof(uint32_t));
                                        }
