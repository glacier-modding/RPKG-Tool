                            ...
                            if (it2.value()["Language"] == languages.at(i) && language_in_locr.at(i)) {
                                ...
                                for (const auto& it3 : it.value().items()) {
                                    if (!it3.value().contains("Language")) {
                                        ...
                                        temp_locr_language_section_string_lengths.push_back(
                                                static_cast<uint32_t>(temp_string.size()));
                                    }
                                }
                                ...
                                locr_language_section_string_hashes.push_back(
                                        temp_locr_language_section_string_hashes);
                                ...
                                if (symKey) {
                                    ...
                                    for (uint32_t k = 0;
                                        ...
                                        std::memcpy(data, &locr_language_section_strings.at(i).at(j)[
                                                    ...
                                                    sizeof(uint32_t));
                                        std::memcpy(data + 1, &locr_language_section_strings.at(i).at(j)[
                                                static_cast<uint64_t>(k) * static_cast<uint64_t>(8) +
                                                static_cast<uint64_t>(4)], sizeof(uint32_t));

                                        crypto::xtea_encrypt_localization(data);

                                        std::memcpy(&locr_language_section_strings.at(i).at(j)[
                                                            static_cast<uint64_t>(k) * static_cast<uint64_t>(8)], data,
                                                    sizeof(uint32_t));
                                        std::memcpy(&locr_language_section_strings.at(i).at(j)[
                                                static_cast<uint64_t>(k) * static_cast<uint64_t>(8) +
