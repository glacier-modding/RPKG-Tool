...
temp::temp(uint64_t rpkgs_index, uint64_t hash_index) {
    ...
    temp_file_name = util::uint64_t_to_hex_string(rpkgs.at(rpkgs_index).hash.at(hash_index).hash_value) + "." +
...
void temp::load_data() {
    ...
    for (rapidjson::SizeType e = 0; e < temp_json_externalSceneTypeIndicesInResourceHeader.Size(); e++) {
        temp_externalSceneTypeIndicesInResourceHeader.push_back(
                temp_json_externalSceneTypeIndicesInResourceHeader[e].GetInt());

        temp_externalSceneHashes.push_back(
                rpkgs.at(temp_rpkg_index).hash.at(temp_hash_index).hash_reference_data.hash_reference.at(
                        temp_externalSceneTypeIndicesInResourceHeader.back()));
    }
    ...
    if (tblu_return_value == TEMP_TBLU_FOUND) {
        ...
        for (rapidjson::SizeType i = 0; i < tblu_json_subEntities.Size(); i++) {
            if (i < temp_logicalParent.size()) {
                ...
                rapidjson::Value::ConstMemberIterator it3 = tblu_json_subEntities[i].FindMember(
...
void temp::load_hash_depends() {
    uint32_t temp_hash_reference_count =
            rpkgs.at(temp_rpkg_index).hash.at(temp_hash_index).hash_reference_data.hash_reference_count & 0x3FFFFFFF;
        ...
        for (uint64_t i = 0; i < temp_hash_reference_count; i++) {
            ...
            for (uint64_t j = 0; j < rpkgs.size(); j++) {
                auto it = rpkgs.at(j).hash_map.find(
                        rpkgs.at(temp_rpkg_index).hash.at(temp_hash_index).hash_reference_data.hash_reference.at(i));

                if (it != rpkgs.at(j).hash_map.end()) {
                    if (rpkgs.at(j).hash.at(it->second).hash_resource_type == "TEMP") {
                        if (!temp_found) {
                            temp_depends_file_name.push_back(
                    ...
                    if (rpkgs.at(j).hash.at(it->second).hash_resource_type == "PRIM") {
                        if (!prim_found) {
                            prim_depends_file_name.push_back(
                                    util::uint64_t_to_hex_string(rpkgs.at(j).hash.at(it->second).hash_value) + "." +
                                    rpkgs.at(j).hash.at(it->second).hash_resource_type);
            ...
            std::string hash_value_string = util::uint64_t_to_hex_string(
                    rpkgs.at(temp_rpkg_index).hash.at(temp_hash_index).hash_reference_data.hash_reference.at(i));
            ...
            if (it2 != hash_list_hash_map.end()) {
                if (hash_value_string == generic_function::compute_ioi_hash(hash_list_hash_strings.at(it2->second))) {
                    ...
                    temp_meta_strings.push_back(util::uint64_t_to_hex_string(
                            rpkgs.at(temp_rpkg_index).hash.at(temp_hash_index).hash_reference_data.hash_reference.at(
                                    i)));
                }
            } else {
            ...
            temp_meta_flags.push_back(util::uint8_t_to_hex_string(
                    rpkgs.at(temp_rpkg_index).hash.at(temp_hash_index).hash_reference_data.hash_reference_type.at(i)));
        }
    }
    ...
    for (uint64_t k = 0; k < temp_depends_file_name.size(); k++) {
        ...
        for (uint64_t d = 0; d < temp_depends_in_rpkgs.at(k).size(); d++) {
            ...
            std::string_view temp_depends_in_rpkgs_string_view(temp_depends_in_rpkgs_upper_case.c_str(),
                                                               temp_depends_in_rpkgs_upper_case.length());
            ...
            if (pos1 != std::string::npos) {
                ...
                if (pos2 != std::string::npos) {
                    ...
                    long new_patch_level = std::strtol(
                            std::string(temp_depends_in_rpkgs_string_view.substr(pos1 + 5, pos2)).c_str(), nullptr, 10);
    ...
    if (tblu_return_value == TEMP_TBLU_FOUND) {
        for (uint64_t k = 0; k < tblu_depends_file_name.size(); k++) {
            ...
            for (uint64_t d = 0; d < tblu_depends_in_rpkgs.at(k).size(); d++) {
                ...
                std::string_view tblu_depends_in_rpkgs_string_view(tblu_depends_in_rpkgs_upper_case.c_str(),
                                                                   tblu_depends_in_rpkgs_upper_case.length());
                ...
                if (pos1 != std::string::npos) {
                    ...
                    if (pos2 != std::string::npos) {
                        ...
                        long new_patch_level = std::strtol(
                                std::string(tblu_depends_in_rpkgs_string_view.substr(pos1 + 5, pos2)).c_str(), nullptr,
                                10);
    ...
    for (uint64_t k = 0; k < prim_depends_file_name.size(); k++) {
        ...
        for (uint64_t d = 0; d < prim_depends_in_rpkgs.at(k).size(); d++) {
            ...
            std::string_view prim_depends_in_rpkgs_string_view(prim_depends_in_rpkgs_upper_case.c_str(),
                                                               prim_depends_in_rpkgs_upper_case.length());
            ...
            if (pos1 != std::string::npos) {
                ...
                if (pos2 != std::string::npos) {
                    ...
                    long new_patch_level = std::strtol(
                            std::string(prim_depends_in_rpkgs_string_view.substr(pos1 + 5, pos2)).c_str(), nullptr, 10);
    ...
    if (tblu_return_value == TEMP_TBLU_FOUND) {
        uint32_t tblu_hash_reference_count =
                rpkgs.at(tblu_rpkg_index).hash.at(tblu_hash_index).hash_reference_data.hash_reference_count &
                0x3FFFFFFF;

        if (tblu_hash_reference_count > 0) {
            for (uint64_t k = 0; k < tblu_hash_reference_count; k++) {
                std::string hash_value_string = util::uint64_t_to_hex_string(
                        rpkgs.at(tblu_rpkg_index).hash.at(tblu_hash_index).hash_reference_data.hash_reference.at(k));
                ...
                if (it2 != hash_list_hash_map.end()) {
                    if (hash_value_string ==
                        ...
                        tblu_meta_strings.push_back(util::uint64_t_to_hex_string(rpkgs.at(tblu_rpkg_index).hash.at(
                                tblu_hash_index).hash_reference_data.hash_reference.at(k)));
                    }
                } else {
                    tblu_meta_strings.push_back(util::uint64_t_to_hex_string(
                            rpkgs.at(tblu_rpkg_index).hash.at(tblu_hash_index).hash_reference_data.hash_reference.at(
                                    k)));
                }

                tblu_meta_flags.push_back(util::uint8_t_to_hex_string(
                        rpkgs.at(tblu_rpkg_index).hash.at(tblu_hash_index).hash_reference_data.hash_reference_type.at(
                                k)));
            }
        }
    }
...
void temp::get_entries_hash_references(uint32_t entry_index) {
    ...
    for (uint32_t e = 0; e < tblu_entityName.size(); e++) {
        if (e == entry_index) {
            ...
            it = hash_list_hash_map.find(
                    rpkgs.at(tblu_rpkg_index).hash.at(tblu_hash_index).hash_reference_data.hash_reference.at(
                            tblu_entityTypeResourceIndex.at(e)));
...
void temp::get_temp_entries_data(const std::string& value_type, std::string type_string) {
    ...
    if (it1 != temp_json_document.MemberEnd()) {
        if (it1->value.IsArray() || it1->value.IsObject()) {
            json_temp_node_scan(it1->value, propertyValues_string, nPropertyID_string, type_string, "/" + value_type,
                                type_string);
        } else {
            std::stringstream ss;
            ...
            if (temp_file_version == 2) {
                ...
                if (it != type_map_h2->end()) {
                    if (it->second == TYPE_INT8) {
                        ...
                        response_string +=
                                util::uint32_t_to_string(it->second) + "Error: The type for " + json_pointer +
...
void temp::json_temp_node_scan(const rapidjson::Value& node, std::string& propertyValues_string,
    ...
    if (node.IsArray()) {
        ...
        for (rapidjson::SizeType i = 0; i < node.Size(); ++i) {
            json_temp_node_scan(node[i], propertyValues_string, nPropertyID_string, type_string,
                                json_pointer + "/" + std::to_string(i), json_type);
        }
    } else if (node.IsObject()) {
        output = false;

        for (rapidjson::Value::ConstMemberIterator it = node.MemberBegin(); it != node.MemberEnd(); it++) {
            json_temp_node_scan(it->value, propertyValues_string, nPropertyID_string, type_string,
                                json_pointer + "/" + it->name.GetString(), json_type + "/" + it->name.GetString());
        }
    } else {
        std::unordered_map<std::string, uint32_t>::iterator it;
        ...
        if (temp_file_version == 2) {
            ...
            if (it != type_map_h2->end()) {
                ...
                if (it->second == TYPE_INT8) {
                    ...
                    response_string += util::uint32_t_to_string(it->second) + "Error: The type for " + json_pointer +
                                       " could not be determined.\n";
                }
                /*if (it->second == 12)
                {
        ...
        for (rapidjson::SizeType p = 0; p < it1->value.Size(); p++) {
            ...
            if (it2 != it1->value[p].MemberEnd()) {
                if (it2->value.IsString()) {
                    ...
                    response_string +=
                            "Error: nPropertyID's type for " + value_type + " (" + util::uint32_t_to_string(p) +
                            ") could not be determined.";
                }
            } else {
                response_string += "Error: nPropertyID for " + value_type + " (" + util::uint32_t_to_string(p) +
                                   ") could not be found.";
            }
            ...
            if (it3 != it1->value[p].MemberEnd()) {
                ...
                if (it4 != it3->value.MemberEnd()) {
                    ...
                    response_string += "Error: $type for " + value_type + " (" + util::uint32_t_to_string(p) +
                                       ") could not be found.";
                }
                ...
                if (it5 != it3->value.MemberEnd()) {
                    if (it5->value.IsArray() || it5->value.IsObject()) {
                        json_node_scan(it5->value, propertyValues_string, nPropertyID_string, type_string,
                                       "/subEntities/" + std::to_string(entry_index) + "/" + value_type + "/" +
                                       std::to_string(p) + "/value/$val", type_string);
                    } else {
                        std::stringstream ss;

                        std::string json_pointer =
                                "/subEntities/" + std::to_string(entry_index) + "/" + value_type + "/" +
                                std::to_string(p) + "/value/$val";
                        ...
                        if (temp_file_version == 2) {
                            ...
                            if (it != type_map_h2->end()) {
                                ...
                                if (it->second == TYPE_INT8) {
                                    ...
                                    response_string += util::uint32_t_to_string(it->second) + "Error: The type for " +
                                                       json_pointer + " could not be determined.\n";
                                }
                                /*if (it->second == 12)
                                {
                                    ss << json_pointer << " ZVariant " << node.GetBool();
                                ...
                                if (it5->value.IsInt64()) {
                                    ...
                                    response_string +=
                                            "Error: The type for " + json_pointer + " could not be determined.\n";
                                }
                            }
                        } else {
                    ...
                    response_string += "Error: $val for " + value_type + " (" + util::uint32_t_to_string(p) +
                                       ") could not be found.";
                }
            } else {
                response_string +=
                        "Error: value for " + value_type + " (" + util::uint32_t_to_string(p) + ") could not be found.";
            }
        }
    } else {
        response_string += "Error: propertyValues for " + value_type + " (" + util::uint32_t_to_string(entry_index) +
                           ") could not be found.";
    }
}

void
temp::json_node_scan(const rapidjson::Value& node, std::string& propertyValues_string, std::string& nPropertyID_string,
    ...
    if (node.IsArray()) {
        ...
        for (rapidjson::SizeType i = 0; i < node.Size(); ++i) {
            json_node_scan(node[i], propertyValues_string, nPropertyID_string, type_string,
                           json_pointer + "/" + std::to_string(i), json_type);
        }
    } else if (node.IsObject()) {
        output = false;
        ...
        for (rapidjson::Value::ConstMemberIterator it = node.MemberBegin(); it != node.MemberEnd(); it++) {
            json_node_scan(it->value, propertyValues_string, nPropertyID_string, type_string,
                           json_pointer + "/" + it->name.GetString(), json_type + "/" + it->name.GetString());
        }
    } else {
        std::unordered_map<std::string, uint32_t>::iterator it;
                ...
                if (it != temps_map.end()) {
                    if (temps.at(it->second).tblu_return_value == TEMP_TBLU_FOUND) {
                        std::unordered_map<uint64_t, uint64_t>::iterator it2 = hash_list_hash_map.find(
                                rpkgs.at(temp_rpkg_index).hash.at(
                                        temp_hash_index).hash_reference_data.hash_reference.at(
                                        temp_depends_index.at(i)));
...
bool temp::rt_json_to_qn_json() {
    ...
    rapidjson::Pointer("/tempHash").Set(qn_json_document, util::uint64_t_to_hex_string(
            rpkgs.at(temp_rpkg_index).hash.at(temp_hash_index).hash_value).c_str());
    rapidjson::Pointer("/tbluHash").Set(qn_json_document, util::uint64_t_to_hex_string(
            rpkgs.at(tblu_rpkg_index).hash.at(tblu_hash_index).hash_value).c_str());

    if (rapidjson::Value* temp_json_value = rapidjson::GetValueByPointer(temp_json_document,
                                                                         rapidjson::Pointer("/rootEntityIndex"))) {
        rapidjson::Pointer("/rootEntity").Set(qn_json_document, util::uint64_t_to_hex_string_for_qn(
                tblu_entityId.at(temp_json_value->GetInt())).c_str());
    }
    ...
    for (uint32_t s = 0; s < temp_subentity_count; s++) {
        ...
        if (rapidjson::Value* tblu_json_logicalParent = rapidjson::GetValueByPointer(tblu_json_document,
                                                                                     rapidjson::Pointer(
                                                                                             json_pointer.c_str()))) {
        ...
        if (rapidjson::Value* temp_json_entityTypeResourceIndex = rapidjson::GetValueByPointer(temp_json_document,
                                                                                               rapidjson::Pointer(
                                                                                                       json_pointer.c_str()))) {
            rapidjson::Pointer("/template").Set(subEntity_json, temp_meta_strings.at(
                    temp_json_entityTypeResourceIndex->GetInt()).c_str());

            if (temp_meta_flags.at(temp_json_entityTypeResourceIndex->GetInt()) != "1F") {
                rapidjson::Pointer("/templateFlag").Set(subEntity_json, temp_meta_flags.at(
                        temp_json_entityTypeResourceIndex->GetInt()).c_str());
            }
        }
        ...
        if (rapidjson::Value* tblu_json_entityTypeResourceIndex = rapidjson::GetValueByPointer(tblu_json_document,
                                                                                               rapidjson::Pointer(
                                                                                                       json_pointer.c_str()))) {
            rapidjson::Pointer("/blueprint").Set(subEntity_json, tblu_meta_strings.at(
                    tblu_json_entityTypeResourceIndex->GetInt()).c_str());
        }
        ...
        if (rapidjson::Value* temp_json_value = rapidjson::GetValueByPointer(tblu_json_document, rapidjson::Pointer(
                json_pointer.c_str()))) {
        ...
        if (rapidjson::Value* temp_json_value = rapidjson::GetValueByPointer(temp_json_document, rapidjson::Pointer(
                json_pointer.c_str()))) {
        ...
        if (rapidjson::Value* temp_json_value = rapidjson::GetValueByPointer(tblu_json_document, rapidjson::Pointer(
            ...
            if (temp_json_value->GetArray().Size() > 0) {
                for (uint32_t a = 0; a < temp_json_value->GetArray().Size(); a++) {
                    json_pointer = "/subEntities/" + util::uint32_t_to_string(s) + "/propertyAliases/" +
                                   util::uint32_t_to_string(a);

                    if (rapidjson::Value* tblu_json_subEntity2 = rapidjson::GetValueByPointer(tblu_json_document,
                                                                                              rapidjson::Pointer(
                                                                                                      json_pointer.c_str()))) {
                        rapidjson::Document json_object(&subEntity_json.GetAllocator());

                        rapidjson::Pointer("/exposeProperty").Set(json_object,
                                                                  tblu_json_subEntity2->GetObject()["sAliasName"].GetString());
                        rapidjson::Pointer("/onEntity").Set(json_object, util::uint64_t_to_hex_string_for_qn(
                                tblu_entityId.at(tblu_json_subEntity2->GetObject()["entityID"].GetInt())).c_str());
                        rapidjson::Pointer("/asProperty").Set(json_object,
                                                              tblu_json_subEntity2->GetObject()["sPropertyName"].GetString());
            ...
            if (temp_json_value->GetArray().Size() > 0) {
                for (uint32_t a = 0; a < temp_json_value->GetArray().Size(); a++) {
                    ...
                    if (rapidjson::Value* tblu_json_subEntity2 = rapidjson::GetValueByPointer(tblu_json_document,
                        ...
                        rapidjson::Pointer("/name").Set(json_object,
                                                        tblu_json_subEntity2->GetObject()["sName"].GetString());
                        rapidjson::Pointer("/isArray").Set(json_object,
                                                           tblu_json_subEntity2->GetObject()["bIsArray"].GetBool());

                        json_pointer = "/subEntities/" + util::uint32_t_to_string(s) + "/exposedEntities/" +
                                       util::uint32_t_to_string(a) + "/aTargets";

                        if (rapidjson::Value* tblu_json_subEntity3 = rapidjson::GetValueByPointer(tblu_json_document,
                                                                                                  rapidjson::Pointer(
                                                                                                          json_pointer.c_str()))) {
                            if (tblu_json_subEntity3->GetArray().Size() > 0) {
                                for (uint32_t t = 0; t < tblu_json_subEntity3->GetArray().Size(); t++) {
                                    json_pointer = "/subEntities/" + util::uint32_t_to_string(s) + "/exposedEntities/" +
                                                   util::uint32_t_to_string(a) + "/aTargets/" +
                                                   util::uint32_t_to_string(t);

                                    if (rapidjson::Value* tblu_json_aTargets = rapidjson::GetValueByPointer(
                                            tblu_json_document, rapidjson::Pointer(json_pointer.c_str()))) {
                                        convert_to_qn_reference(json_object, tblu_json_aTargets->GetObject(),
                                                                "/targets/" + util::uint32_t_to_string(t));
                                    }
                                }
                            } else {
            ...
            if (temp_json_value->GetArray().Size() > 0) {
                for (uint32_t a = 0; a < temp_json_value->GetArray().Size(); a++) {
                    ...
                    if (rapidjson::Value* tblu_json_subEntity2 = rapidjson::GetValueByPointer(tblu_json_document,
                        ...
                        json_pointer = "/subEntities/" + util::uint32_t_to_string(s) + "/exposedInterfaces/" +
                                       util::uint32_t_to_string(a) + "/1";

                        rapidjson::Pointer(json_pointer.c_str()).Set(tblu_json_document,
                                                                     util::uint64_t_to_hex_string_for_qn(
                                                                             tblu_entityId.at(
                                                                                     tblu_json_subEntity2->GetArray()[1].GetInt())).c_str());
                    }
                }
        ...
        if (it != tblu_json_subEntities[s].MemberEnd()) {
            for (uint64_t e = 0; e < it->value.Size(); e++) {
                for (uint64_t a = 0; a < it->value[e][1]["entities"].Size(); a++) {
                    std::string entityId_string = util::uint64_t_to_hex_string_for_qn(
                            tblu_entityId.at(it->value[e][1]["entities"][a].GetInt()));

                    it->value[e][1]["entities"][a].SetString(entityId_string.c_str(), entityId_string.length(),
                                                             qn_json_document.GetAllocator());
                }
            }
        ...
        if (rapidjson::Value* temp_json_value = rapidjson::GetValueByPointer(subEntity_json, rapidjson::Pointer(
                json_pointer.c_str()))) {
            if (temp_json_value->GetArray().Size() > 0) {
                for (uint32_t a = 0; a < temp_json_value->GetArray().Size(); a++) {
                    ...
                    rapidjson::Pointer(json_pointer.c_str()).Set(subEntity_json,
                                                                 temp_json_value->GetArray()[a]["value"]["$type"]);

                    json_pointer = "/properties/" + nPropertyID + "/value";

                    rapidjson::Pointer(json_pointer.c_str()).Set(subEntity_json,
                                                                 temp_json_value->GetArray()[a]["value"]["$val"]);
                }
            }
        }
    ...
    if (rapidjson::Value* temp_json_value = rapidjson::GetValueByPointer(tblu_json_document,
                                                                         rapidjson::Pointer(json_pointer.c_str()))) {
        if (temp_json_value->GetArray().Size() > 0) {
            for (uint32_t a = 0; a < temp_json_value->GetArray().Size(); a++) {
                rapidjson::Document json_object(&qn_json_document.GetAllocator());

                json_pointer = "/entities/" + util::uint64_t_to_hex_string_for_qn(
                        tblu_entityId.at(temp_json_value->GetArray()[a]["fromID"].GetInt()));

                rapidjson::Pointer("/onEvent").Set(json_object,
                                                   temp_json_value->GetArray()[a]["fromPinName"].GetString());
                rapidjson::Pointer("/shouldTrigger").Set(json_object,
                                                         temp_json_value->GetArray()[a]["toPinName"].GetString());
                rapidjson::Pointer("/onEntity").Set(json_object, util::uint64_t_to_hex_string_for_qn(
                        tblu_entityId.at(temp_json_value->GetArray()[a]["toID"].GetInt())).c_str());

                if (temp_json_value->GetArray()[a].HasMember("constantPinValue")) {
                    if (std::strcmp(temp_json_value->GetArray()[a]["constantPinValue"]["$type"].GetString(), "void") !=
                        0) {
                        rapidjson::Pointer("/value/type").Set(json_object,
                                                              temp_json_value->GetArray()[a]["constantPinValue"]["$type"].GetString());
                        rapidjson::Pointer("/value/value").Set(json_object,
                                                               temp_json_value->GetArray()[a]["constantPinValue"]["$val"]);
                    }
                }

                json_pointer = "/entities/" + util::uint64_t_to_hex_string_for_qn(
                        tblu_entityId.at(temp_json_value->GetArray()[a]["fromID"].GetInt())) + "/events";
    ...
    if (rapidjson::Value* temp_json_value = rapidjson::GetValueByPointer(tblu_json_document,
        ...
        if (temp_json_value->GetArray().Size() > 0) {
            for (uint32_t a = 0; a < temp_json_value->GetArray().Size(); a++) {
                ...
                rapidjson::Pointer("/whenTriggered").Set(json_object,
                                                         temp_json_value->GetArray()[a]["fromPinName"].GetString());
                rapidjson::Pointer("/alsoTrigger").Set(json_object,
                                                       temp_json_value->GetArray()[a]["toPinName"].GetString());
                rapidjson::Pointer("/onEntity").Set(json_object, util::uint64_t_to_hex_string_for_qn(
                        tblu_entityId.at(temp_json_value->GetArray()[a]["toID"].GetInt())).c_str());
                ...
                json_pointer = "/entities/" + util::uint64_t_to_hex_string_for_qn(
                        tblu_entityId.at(temp_json_value->GetArray()[a]["fromID"].GetInt())) + "/inputCopying";
    ...
    if (rapidjson::Value* temp_json_value = rapidjson::GetValueByPointer(tblu_json_document,
        ...
        if (temp_json_value->GetArray().Size() > 0) {
            for (uint32_t a = 0; a < temp_json_value->GetArray().Size(); a++) {
                ...
                rapidjson::Pointer("/onEvent").Set(json_object,
                                                   temp_json_value->GetArray()[a]["fromPinName"].GetString());
                rapidjson::Pointer("/propagateEvent").Set(json_object,
                                                          temp_json_value->GetArray()[a]["toPinName"].GetString());
                rapidjson::Pointer("/onEntity").Set(json_object, util::uint64_t_to_hex_string_for_qn(
                        tblu_entityId.at(temp_json_value->GetArray()[a]["toID"].GetInt())).c_str());
                ...
                json_pointer = "/entities/" + util::uint64_t_to_hex_string_for_qn(
                        tblu_entityId.at(temp_json_value->GetArray()[a]["fromID"].GetInt())) + "/outputCopying";
    ...
    if (rapidjson::Value* temp_json_value = rapidjson::GetValueByPointer(temp_json_document,
        ...
        if (temp_json_value->GetArray().Size() > 0) {
            for (uint32_t a = 0; a < temp_json_value->GetArray().Size(); a++) {
                ...
                convert_to_qn_reference(property_overrides_json_object,
                                        temp_json_value->GetArray()[a].GetObject()["propertyOwner"].GetObject(),
                                        "/entities");

                convert_to_qn_property(property_overrides_json_object,
                                       temp_json_value->GetArray()[a].GetObject()["propertyValue"]);
                ...
                if (temp_json_value->GetArray()[a].GetObject()["propertyValue"]["nPropertyID"].IsString()) {
                    ...
                    nPropertyID = util::uint32_t_to_string(
                            temp_json_value->GetArray()[a].GetObject()["propertyValue"]["nPropertyID"].GetUint());
                }

                json_pointer = "/properties";

                if (!rapidjson::GetValueByPointer(property_overrides_json_object,
                                                  rapidjson::Pointer(json_pointer.c_str()))) {
                    rapidjson::Pointer(json_pointer.c_str()).Set(property_overrides_json_object,
                ...
                rapidjson::Pointer(json_pointer.c_str()).Set(property_overrides_json_object,
                                                             temp_json_value->GetArray()[a].GetObject()["propertyValue"]["value"]["$type"]);

                json_pointer = "/properties/" + nPropertyID + "/value";

                rapidjson::Pointer(json_pointer.c_str()).Set(property_overrides_json_object,
                                                             temp_json_value->GetArray()[a].GetObject()["propertyValue"]["value"]["$val"]);
...
//    }
    ...
    if (rapidjson::Value* temp_json_value = rapidjson::GetValueByPointer(tblu_json_document,
                                                                         rapidjson::Pointer("/overrideDeletes"))) {
    ...
    if (rapidjson::Value* entities_json_value = rapidjson::GetValueByPointer(entities_json_object,
                                                                             rapidjson::Pointer(""))) {
        if (rapidjson::Value* temp_json_value = rapidjson::GetValueByPointer(qn_json_document, rapidjson::Pointer(
                "/propertyOverrides"))) {
            if (temp_json_value->GetArray().Size() > 0) {
                for (uint32_t a = 0; a < temp_json_value->GetArray().Size(); a++) {
                    ...
                    for (uint32_t e = 0; e < entities_json_value->GetArray().Size(); e++) {
                        if (temp_json_value->GetArray()[a].GetObject()["entities"] ==
                            entities_json_value->GetArray()[e]["entities"]) {
                            in_entities = true;

                            json_pointer = "/" + util::uint32_t_to_string(e) + "/properties";

                            if (rapidjson::Value* properties_json_value = rapidjson::GetValueByPointer(
                                    entities_json_object, rapidjson::Pointer(json_pointer.c_str()))) {
                                std::string property_name = "";

                                for (rapidjson::Value::ConstMemberIterator iter = temp_json_value->GetArray()[a].GetObject()["properties"].MemberBegin();
                                     iter !=
                                     temp_json_value->GetArray()[a].GetObject()["properties"].MemberEnd(); ++iter) {
                                    property_name = std::string(iter->name.GetString());
                                }

                                rapidjson::Value key(property_name.c_str(), entities_json_object.GetAllocator());

                                properties_json_value->AddMember(key,
                                                                 temp_json_value->GetArray()[a].GetObject()["properties"].GetObject()[key],
                                                                 entities_json_object.GetAllocator());
                            }
                        }
                    }
                    ...
                    if (!in_entities) {
                        ...
                        rapidjson::Pointer(json_pointer.c_str()).Set(entities_json_object,
                                                                     temp_json_value->GetArray()[a].GetObject()["entities"]);

                        json_pointer = "/" + util::uint32_t_to_string(array_index) + "/properties";
                        rapidjson::Pointer(json_pointer.c_str()).Set(entities_json_object,
                                                                     temp_json_value->GetArray()[a].GetObject()["properties"]);
                    }
                }
            }
        }

        if (rapidjson::Value* properties_json_value = rapidjson::GetValueByPointer(properties_json_object,
                                                                                   rapidjson::Pointer(""))) {
            for (uint32_t e = 0; e < entities_json_value->GetArray().Size(); e++) {
                bool in_properties = false;

                for (uint32_t p = 0; p < properties_json_value->GetArray().Size(); p++) {
                    if (entities_json_value->GetArray()[e].GetObject()["properties"] ==
                        properties_json_value->GetArray()[p]["properties"]) {
                        in_properties = true;

                        json_pointer = "/" + util::uint32_t_to_string(p) + "/entities";

                        if (rapidjson::Value* properties_json = rapidjson::GetValueByPointer(properties_json_object,
                                                                                             rapidjson::Pointer(
                                                                                                     json_pointer.c_str()))) {
                            properties_json->PushBack(entities_json_value->GetArray()[e].GetObject()["entities"],
                                                      properties_json_object.GetAllocator());
                        }
                    }
                }
                ...
                if (!in_properties) {
                    ...
                    rapidjson::Pointer(json_pointer.c_str()).Set(properties_json_object,
                                                                 entities_json_value->GetArray()[e].GetObject()["entities"]);

                    json_pointer = "/" + util::uint32_t_to_string(array_index) + "/properties";
                    rapidjson::Pointer(json_pointer.c_str()).Set(properties_json_object,
                                                                 entities_json_value->GetArray()[e].GetObject()["properties"]);
                }
            }
        }
    }
    ...
    if (rapidjson::Value* temp_json_value = rapidjson::GetValueByPointer(qn_json_document,
                                                                         rapidjson::Pointer("/overrideDeletes"))) {
        if (temp_json_value->GetArray().Size() > 0) {
            for (uint32_t a = 0; a < temp_json_value->GetArray().Size(); a++) {
    ...
    for (uint32_t s = 0; s < temp_externalSceneTypeIndicesInResourceHeader.size(); s++) {
        ...
        rapidjson::Pointer(json_pointer.c_str()).Set(qn_json_document, temp_meta_strings.at(
                temp_externalSceneTypeIndicesInResourceHeader.at(s)).c_str());
    }

    if (rapidjson::Value* temp_json_value = rapidjson::GetValueByPointer(temp_json_document,
                                                                         rapidjson::Pointer("/subType"))) {
...
void temp::convert_to_qn_reference(rapidjson::Document& json_document, rapidjson::Value& reference,
    ...
    if (externalSceneIndex != -1 || exposedEntity.length() > 0) {
        ...
        if (entityIndex == -1) {
            ...
            rapidjson::Pointer("/ref").Set(json_object,
                                           util::uint64_t_to_hex_string_for_qn(tblu_entityId.at(entityIndex)).c_str());
        }

        if (externalSceneIndex == -1) {
            rapidjson::Pointer("/externalScene").Set(json_object, rapidjson::kNullType);
        } else {
            rapidjson::Pointer("/externalScene").Set(json_object, temp_meta_strings.at(
                    temp_externalSceneTypeIndicesInResourceHeader.at(externalSceneIndex)).c_str());
        }
        ...
        if (entityIndex == -1) {
            ...
            rapidjson::Pointer(pointer_string.c_str()).Set(json_document, util::uint64_t_to_hex_string_for_qn(
                    tblu_entityId.at(entityIndex)).c_str());
        }
    }
}
...
void temp::convert_to_qn_property(rapidjson::Document& json_document, rapidjson::Value& property) {
    if (property["nPropertyID"].IsString()) {
            ...
            if (type.substr(0, 7) == "TArray<") {
                if (property["value"]["$val"].GetArray().Size() > 0) {
                    for (uint32_t a = 0; a < property["value"]["$val"].GetArray().Size(); a++) {
                        ...
                        rapidjson::Pointer("/value/$type").Set(json_object,
                                                               type.substr(7, (type.length() - 8)).c_str());
        ...
        if (type == "SEntityTemplateReference") {
            ...
            if (property["value"]["$val"]["m_IDLow"].GetUint() == 4294967295 &&
                property["value"]["$val"]["m_IDHigh"].GetUint() == 4294967295) {
                rapidjson::Pointer("").Set(json_object, rapidjson::kNullType);
            } else {
                if (temp_meta_flags.at(property["value"]["$val"]["m_IDLow"].GetUint()) != "1F") {
                    rapidjson::Pointer("/resource").Set(json_object, temp_meta_strings.at(
                            property["value"]["$val"]["m_IDLow"].GetUint()).c_str());
                    rapidjson::Pointer("/flag").Set(json_object, temp_meta_flags.at(
                            property["value"]["$val"]["m_IDLow"].GetUint()).c_str());
                } else {
                    rapidjson::Pointer("").Set(json_object, temp_meta_strings.at(
                            property["value"]["$val"]["m_IDLow"].GetUint()).c_str());
                }
            }
            ...
            ss << "#" << std::hex << std::setw(2) << std::setfill('0')
               << (uint32_t) std::round(property["value"]["$val"]["r"].GetFloat() * 255.0f);
            ss << std::hex << std::setw(2) << std::setfill('0')
               << (uint32_t) std::round(property["value"]["$val"]["g"].GetFloat() * 255.0f);
            ss << std::hex << std::setw(2) << std::setfill('0')
               << (uint32_t) std::round(property["value"]["$val"]["b"].GetFloat() * 255.0f);
            ...
            ss << std::hex << std::setw(2) << std::setfill('0')
               << (uint32_t) std::round(property["value"]["$val"]["b"].GetFloat() * 255.0f);
            ss << std::hex << std::setw(2) << std::setfill('0')
               << (uint32_t) std::round(property["value"]["$val"]["a"].GetFloat() * 255.0f);
...
void temp::generate_qn_patch_json() {
    ...
    try {
        qn_json_original = json::parse(buffer_qn_original.GetString(),
                                       buffer_qn_original.GetString() + buffer_qn_original.GetSize());

        qn_json = json::parse(buffer_qn.GetString(), buffer_qn.GetString() + buffer_qn.GetSize());

        qn_patch_json["tempHash"] = util::uint64_t_to_hex_string(
                rpkgs.at(temp_rpkg_index).hash.at(temp_hash_index).hash_value);
        qn_patch_json["tbluHash"] = util::uint64_t_to_hex_string(
                rpkgs.at(tblu_rpkg_index).hash.at(tblu_hash_index).hash_value);
