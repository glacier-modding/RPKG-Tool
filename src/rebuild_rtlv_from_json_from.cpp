        ...
        for (uint64_t p = 0; p < json_file_paths.size(); p++)
        {
            ...
            if (file::path_exists(json_file_paths.at(p) + ".meta"))
            {
                ...
                for (const auto& it : input_json.items())
                {
                    ...
                    if (!language_found)
                    {
                        LOG_AND_EXIT("Error: JSON file " + json_file_paths.at(p) + " is malformed and can not be rebuilt into a RTLV file/resource.");
                    }
                }
                ...
                {
                    LOG_AND_EXIT("Error: Number of language in the input RTLV JSON file do not match the number of languages in the meta file.");
                }
                ...
                std::ofstream output_file = std::ofstream(current_path + "\\" + rtlv_file_names.at(p), std::ofstream::binary);
