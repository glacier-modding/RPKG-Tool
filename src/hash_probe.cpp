...
void rpkg_function::hash_probe(const std::string& input_path, const std::string& filter) {
    ...
    if (!file::path_exists(input_rpkg_folder_path)) {
        LOG_AND_EXIT("Error: The folder " + input_rpkg_folder_path +
                     " to search for RPKG files for hash probe mode does not exist.");
    }
    ...
    for (auto& filter : filters) {
        ...
        if (hash != 0) {
            ...
            for (auto& rpkg : rpkgs) {
                ...
                ss << "  - Hash reference table size: " << rpkg.hash.at(it2->second).data.resource.reference_table_size
                   << std::endl;
                ss << "  - Forward hash depends: "
                   << (rpkg.hash.at(it2->second).hash_reference_data.hash_reference_count & 0x3FFFFFFF) << std::endl;
                ...
                ss << "  - Size in video memory: " << rpkg.hash.at(it2->second).data.resource.size_in_video_memory
                   << std::endl << std::endl;
            }
