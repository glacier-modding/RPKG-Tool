...
void rpkg_function::hash_list_percent_found() {
    if (!hash_list_loaded) {
    ...
    std::cout << "The percentage of hashes in the hash list that were matched: "
              << static_cast<double>(matched) / static_cast<double>(hash_list_hash_value_strings.size()) * 100.0 << "%"
              << std::endl;
}
