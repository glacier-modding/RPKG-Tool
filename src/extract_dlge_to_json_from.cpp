            ...
            for (uint64_t j = 0; j < rpkgs.at(i).hashes_indexes_based_on_resource_types.at(r).size(); j++) {
                    ...
                    while (!done_scanning_languages) {
                        ...
                        for (uint32_t m = 0; m < temp_language_string_sizes.back() / 8; m++) {
                            ...
                            std::memcpy(data, &temp_string[static_cast<uint64_t>(m) * static_cast<uint64_t>(8)],
                                        sizeof(uint32_t));
                            std::memcpy(data + 1, &temp_string[static_cast<uint64_t>(m) * static_cast<uint64_t>(8) +
                ...
                if (output_to_string) {
                    ...
                    for (uint64_t a = 0; a < rpkgs.size(); a++) {
                        auto it2 = rpkgs.at(a).hash_map.find(hash_value);
                        ...
                        uint32_t temp_hash_reference_count =
                                rpkgs.at(a).hash.at(it2->second).hash_reference_data.hash_reference_count & 0x3FFFFFFF;
                        ...
                        if (temp_hash_reference_count > 0) {
                            for (uint64_t b = 0; b < temp_hash_reference_count; b++) {
                                ...
                                for (uint64_t c = 0; c < rpkgs.size(); c++) {
                                    auto it3 = rpkgs.at(c).hash_map.find(
                                            rpkgs.at(a).hash.at(it2->second).hash_reference_data.hash_reference.at(b));
                                    ...
                                    if (!found) {
                                        temp_hash_depends_data.hash_dependency_file_name.push_back(
                                                util::uint64_t_to_hex_string(
                                                        rpkgs.at(c).hash.at(it3->second).hash_value) + "." +
                                                rpkgs.at(c).hash.at(it3->second).hash_resource_type);

                                        std::string hash_link_file_name = current_path + "\\" + hash_file_name + "_" +
                                                                          util::uint64_t_to_hex_string(
                                                                                  rpkgs.at(c).hash.at(
                                        ...
                                        if (rpkgs.at(c).hash.at(it3->second).hash_resource_type == "WWES") {
                                            if (!file::path_exists(hash_link_file_name)) {
                                                std::ofstream json_meta_hash_link_file = std::ofstream(
                                                        hash_link_file_name, std::ofstream::binary);

                                                if (!json_meta_hash_link_file.good()) {
                                                    LOG_AND_EXIT("Error: JSON meta file " + hash_link_file_name +
                                ...
                                if (!found) {
                                    temp_hash_depends_data.hash_dependency_file_name.push_back(
                                            util::uint64_t_to_hex_string(rpkgs.at(a).hash.at(
                                                    it2->second).hash_reference_data.hash_reference.at(b)));
                                }

                                temp_hash_depends_data.hash_dependency_map[hash_value] = temp_hash_depends_data.hash_dependency_map.size();

                                temp_hash_depends_data.hash_dependency.push_back(
                                        util::uint64_t_to_hex_string(hash_value));
