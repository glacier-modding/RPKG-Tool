...
void rpkg_function::extract_from_rpkg_with_map(rpkg_extraction_vars& rpkg_vars) {
    ...
    for (uint64_t i = 0; i < rpkgs.size(); i++) {
        ...
        std::string hash_file_name = util::uint64_t_to_hex_string(rpkgs.at(i).hash.at(it->second).hash_value) + "." +
                                     rpkgs.at(i).hash.at(it->second).hash_resource_type;
        ...
        if (rpkgs.at(i).hash.at(it->second).data.lz4ed) {
            LZ4_decompress_safe(input_data.data(), lz4_output_data.data(), static_cast<int>(hash_size),
                                decompressed_size);
