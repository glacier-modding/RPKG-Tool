...
material::material(uint64_t rpkgs_index, uint64_t hash_index)
{
    ...
    std::string mati_hash_file_name = util::uint64_t_to_hex_string(rpkgs.at(mati_rpkg_index).hash.at(mati_hash_index).hash_value) + "." + rpkgs.at(mati_rpkg_index).hash.at(mati_hash_index).hash_resource_type;
            ...
            for (uint64_t b = 0; b < rpkgs.at(a).hash.size(); b++)
            {
                if (!(matt_hash_value && matb_hash_value))
                {
                    ...
                    for (uint64_t c = 0; c < temp_hash_reference_count; c++)
                    {
                        ...
                        {
                            if (rpkgs.at(a).hash.at(b).hash_reference_data.hash_reference.at(c) == rpkgs.at(rpkgs_index).hash.at(hash_index).hash_value)
                            {
                                matt_hash_value = rpkgs.at(a).hash.at(b).hash_value;

                                uint32_t matt_hash_reference_count = rpkgs.at(a).hash.at(b).hash_reference_data.hash_reference_count & 0x3FFFFFFF;
                                ...
                                if (matt_hash_reference_count > 0)
                                {
                                    for (uint64_t i = 0; i < matt_hash_reference_count; i++)
                                    {
                                        for (uint64_t j = 0; j < rpkgs.size(); j++)
                                        {
                                            if (!(matt_hash_value && matb_hash_value))
                                            {
                                                std::unordered_map<uint64_t, uint64_t>::iterator it = rpkgs.at(j).hash_map.find(rpkgs.at(a).hash.at(b).hash_reference_data.hash_reference.at(i));
    ...
    if (matt_hash_value && matb_hash_value)
    {
        ...
        if (rpkg_index != UINT32_MAX)
        {
            ...
            if (it6 != rpkgs.at(rpkg_index).hash_map.end())
            {
                ...
                {
                    std::unordered_map<uint64_t, uint64_t>::iterator it4 = rpkgs.at(rpkg_index2).hash_map.find(matb_hash_value);
                    ...
                    if (it4 != rpkgs.at(rpkg_index2).hash_map.end())
                    {
                        ...
                        {
                            matt_hash_size = rpkgs.at(matt_rpkg_index).hash.at(matt_hash_index).data.resource.size_final;
                        }

                        std::vector<char> matt_input_data = std::vector<char>(matt_hash_size, 0);

                        std::ifstream file = std::ifstream(rpkgs.at(matt_rpkg_index).rpkg_file_path, std::ifstream::binary);

                        if (!file.good())
                        {
                            LOG_AND_EXIT("Error: RPKG file " + rpkgs.at(matt_rpkg_index).rpkg_file_path + " could not be read.");
                        }

                        file.seekg(rpkgs.at(matt_rpkg_index).hash.at(matt_hash_index).data.header.data_offset, file.beg);
                        ...
                        uint32_t matt_decompressed_size = rpkgs.at(matt_rpkg_index).hash.at(matt_hash_index).data.resource.size_final;

                        std::vector<char> matt_output_data = std::vector<char>(matt_decompressed_size, 0);

                        if (rpkgs.at(matt_rpkg_index).hash.at(matt_hash_index).data.lz4ed)
                        {
                            LZ4_decompress_safe(matt_input_data.data(), matt_output_data.data(), (int)matt_hash_size, matt_decompressed_size);
                        ...
                        std::string matb_hash_file_name = util::uint64_t_to_hex_string(rpkgs.at(matb_rpkg_index).hash.at(matb_hash_index).hash_value) + "." + rpkgs.at(matb_rpkg_index).hash.at(matb_hash_index).hash_resource_type;
                        ...
                        {
                            matb_hash_size = rpkgs.at(matb_rpkg_index).hash.at(matb_hash_index).data.resource.size_final;
                        }

                        std::vector<char> matb_input_data = std::vector<char>(matb_hash_size, 0);

                        std::ifstream file2 = std::ifstream(rpkgs.at(matb_rpkg_index).rpkg_file_path, std::ifstream::binary);

                        if (!file2.good())
                        {
                            LOG_AND_EXIT("Error: RPKG file " + rpkgs.at(matb_rpkg_index).rpkg_file_path + " could not be read.");
                        }

                        file2.seekg(rpkgs.at(matb_rpkg_index).hash.at(matb_hash_index).data.header.data_offset, file2.beg);
                        ...
                        uint32_t matb_decompressed_size = rpkgs.at(matb_rpkg_index).hash.at(matb_hash_index).data.resource.size_final;

                        std::vector<char> matb_output_data = std::vector<char>(matb_decompressed_size, 0);

                        if (rpkgs.at(matb_rpkg_index).hash.at(matb_hash_index).data.lz4ed)
                        {
                            LZ4_decompress_safe(matb_input_data.data(), matb_output_data.data(), (int)matb_hash_size, matb_decompressed_size);
    ...
    if (matt_hash_value && matb_hash_value)
    {
        ...
        while (position < matt_data.size())
        {
            ...
            if (matt_type == 1)
            {
                ...
                std::memcpy(&texture_hash_depends_index, &matt_data.data()[position], sizeof(texture_hash_depends_index));
        ...
        for (uint32_t i = 0; i < matt_entries.size(); i++)
        {
            if (matt_entries[i].index() == 0)
            {
                ...
                {
                    json["Overrides"]["Texture"][matb_entries[i]] = util::hash_to_ioi_string(rpkgs.at(matt_rpkg_index).hash.at(matt_hash_index).hash_reference_data.hash_reference.at(std::get<int32_t>(matt_entries[i])), true);
...
nlohmann::ordered_json material::read_mati_properties_json(nlohmann::ordered_json temp_json, uint32_t position, uint32_t parent, bool from_mati_file)
{
    uint8_t bytes1 = 0;
    uint32_t bytes4 = 0;
    uint64_t bytes8 = 0;
    ...
    if (log_output)
    {
        std::cout << "MATI temp_mati_property.name: " << temp_mati_property.name << " (" << mati_property_name << ")" << std::endl;
    ...
    else if (temp_mati_property.type == 2)
    {
        ...
        if (temp_mati_property.name == "TXID")
        {
            ...
            {
                temp_json[mati_property_name] = util::hash_to_ioi_string(rpkgs.at(mati_rpkg_index).hash.at(mati_hash_index).hash_reference_data.hash_reference.at(temp_mati_property.int_value), true);
            }
        }
        else
        {
...
material::material(std::string json_path, std::string output_path)
{
    ...
    try
    {
            ...
            for (uint32_t i = 0; i < json["Material"]["Instance"][0]["Binder"][0]["Texture"].size(); i++)
            {
                if (json["Material"]["Instance"][0]["Binder"][0]["Texture"][i].contains("Texture Id"))
                {
                    if (json["Material"]["Instance"][0]["Binder"][0]["Texture"][i].contains("Name"))
                    {
                        ...
                        mati_textures.push_back(std::string(json["Material"]["Instance"][0]["Binder"][0]["Texture"][i]["Name"]));
                    }
                }
            }
        }
            ...
            if (json.contains("Overrides"))
            {
                if (json["Overrides"].contains("Texture"))
                {
                    for (const auto& it : json["Overrides"]["Texture"].items())
                    {
                        ...
                        {
                            if (std::find(mati_textures.begin(), mati_textures.end(), text_name) != mati_textures.end())
                            {
        ...
        mati_file = std::ofstream(file::output_path_append(util::uint64_t_to_hex_string(mati_hash_value) + ".MATI", output_path), std::ofstream::binary);
        mati_meta_file = std::ofstream(file::output_path_append(util::uint64_t_to_hex_string(mati_hash_value) + ".MATI.meta", output_path), std::ofstream::binary);

        if (matt_hash_value && matb_hash_value)
        {
            matt_file = std::ofstream(file::output_path_append(util::uint64_t_to_hex_string(matt_hash_value) + ".MATT", output_path), std::ofstream::binary);
            matt_meta_file = std::ofstream(file::output_path_append(util::uint64_t_to_hex_string(matt_hash_value) + ".MATT.meta", output_path), std::ofstream::binary);
            matb_file = std::ofstream(file::output_path_append(util::uint64_t_to_hex_string(matb_hash_value) + ".MATB", output_path), std::ofstream::binary);
            matb_meta_file = std::ofstream(file::output_path_append(util::uint64_t_to_hex_string(matb_hash_value) + ".MATB.meta", output_path), std::ofstream::binary);
        }
