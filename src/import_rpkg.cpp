...
void rpkg_function::import_rpkg(std::string& rpkg_file_path, bool with_timing) {
    ...
    if ((rpkgs.back().rpkg_file_version == 1 && rpkg_file.Size() <= 0x14) ||
        (rpkgs.back().rpkg_file_version == 2 && rpkg_file.Size() <= 0x1D)) {
        LOG_AND_RETURN("Error: " + rpkg_file_path + " is a empty RPKG file.");
    }
    ...
    if (rpkgs.back().rpkg_file_version == 1 && (rpkgs.back().header.patch_count * 8 + 0x24) >= rpkg_file.Size())
        ...
        if (rpkgs.back().rpkg_file_version == 1 && test_header_offset == (rpkgs.back().header.hash_header_table_size +
                                                                          rpkgs.back().header.hash_resource_table_size +
                                                                          rpkgs.back().header.patch_count * 8 + 0x14) &&
            test_zero_value == 0)
            rpkgs.back().is_patch_file = true;
        else if (test_header_offset ==
                 (rpkgs.back().header.hash_header_table_size + rpkgs.back().header.hash_resource_table_size +
                  rpkgs.back().header.patch_count * 8 + 0x1D) && test_zero_value == 0)
            rpkgs.back().is_patch_file = true;
    }
    ...
    size_t hash_tables_combined_size =
            rpkgs.back().header.hash_header_table_size + rpkgs.back().header.hash_resource_table_size;
    ...
    for (size_t i = 0; i < rpkgs.back().header.hash_count; i++) {
        ...
        if (!rpkgs.back().hash_resource_types.empty()) {
            ...
            if (!found) {
                ...
                rpkgs.back().hashes_indexes_based_on_resource_types.push_back(
                        temp_hashes_indexes_based_on_resource_types);
        ...
        if (rpkgs.back().hash[i].data.resource.reference_table_size > 0) {
            ...
            for (uint64_t j = 0; j < depends_count; j++) {
                auto it = hashes_depends_map.back().find(
                        rpkgs.back().hash[i].hash_reference_data.hash_reference.back());
