...
void rpkg_function::extract_mati_to_json(std::string& input_path, const std::string& filter, const std::string& output_path)
{
    ...
    for (auto& filter : filters)
    {
        ...
        for (uint64_t i = 0; i < rpkgs.size(); i++)
        {
            ...
            timing_string = "Converting: " + util::uint64_t_to_hex_string(rpkgs.at(rpkg_index).hash.at(it->second).hash_value) + "." + rpkgs.at(rpkg_index).hash.at(it->second).hash_resource_type + " to MATI JSON";
